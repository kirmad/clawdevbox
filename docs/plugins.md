...
```json
{
  ...
  "homepage": "https://example.com/calls",
  "repository": "https://github.com/example/calls",
  "license": "MIT",
  ...
  "status": {
    ...
    "experimental": false,
    "notes": "Requires az login to obtain the Calls audience token."
  },
  ...
  "clawdevbox": {
    ...
    "tools": [
      { "id": "calls.fetch_call", "file": "tools/fetch_call.ts", "runtime": "tsx" }
    ],
    "trigger_types": [
      {
        "id": "calls.audit-watcher",
...
```bash
clawdevbox init --plugin git+https://github.com/example/calls-plugin
...
clawdevbox plugin install calls@clawdevbox-plugins
```

The first command persists the catalog under `<globalDir>/marketplaces/`.
The second resolves the source for `calls` from the catalog and installs
...
```json
{ "from": "git+https://github.com/example/calls-plugin" }
```
...
…then pick "Agency" in the agent-CLI chooser. See
