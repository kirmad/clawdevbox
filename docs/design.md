...
CREATE TABLE inbox_items (
  ...
  source          TEXT NOT NULL,               -- ado | github | incidents | manual
...
CREATE TABLE messages (
  ...
  ts          INTEGER NOT NULL,
  attribution TEXT                             -- e.g. "user:slack:@devuser", "agent:claude", "policy:stuck-pr"
);
...
kind: incident
default_client: claude
mcp_servers: [incidents, mdm, logsearch, conductor]
timeout_minutes: 0
steps:
  - id: 1
    goal: "Read the incident details (incidents.get_incident). Update the inbox card with severity + service. Set agent_tone='err'."
  - id: 2
    goal: "Form 2-4 hypotheses ranked by likelihood (recent deploys, ongoing dependencies, similar past incidents from memory)."
    depends: [1]
  - id: 3
    goal: "For each hypothesis, gather evidence in parallel: query MDM for relevant metrics, LogSearch for error-log signals, and Incidents for related incidents. Use thread.spawn(parent_thread_id=<self>, recipe_id='gather-evidence-for-hypothesis', prompt=...) for each, run them concurrently."
    depends: [2]
  - id: 4
...
steps:
  - id: 1
    goal: "Identify the queries needed for this hypothesis (which MDM metric, which LogSearch filter, which time window)."
  - id: 2
...
mcp_servers: [incidents, mdm, terminal, conductor]
steps:
  - id: 1
...
| Mechanism | When | How |
|---|---|---|
| **External webhook** (real-time) | ADO PR pushed, Incidents incident fired, GitHub Action completes, Slack receives a message | The external system is configured to POST to `http://localhost:5201/hooks/<id>` (typically via a tunnel or service-hook config). Sub-second latency. |
...
| Request body | Optional JSON. Whatever the caller wants to pass to the script. ADO service hooks send PR payloads; Incidents sends incident payloads; cron self-fires send `{}`. |
...
```json
{
  ...
  "state": { "lastCommentId": 1234, "selfUser": "devuser@example.com" },
  "payload": {
...
Each sample is two parts: the entry in `triggers.json` (declares the webhook + handler binding) and the executable script file (runs whenever the webhook fires). Scripts handle both **cron-fired** (empty stdin, poll for state) and **externally-fired** (stdin has payload from ADO/Incidents/whatever) cases — the dual-fire pattern.
...
const input = JSON.parse(await readStdin());
const state = { lastCheckedAt: 0, selfUser: 'devuser@example.com', ...input.state };
...
```json
{
  "id": "ado.comment-watcher#2401",
  "type": "ado.comment-watcher",
  "params": { "repo": "auth-svc", "pr_id": 2401, "self_user": "devuser@example.com" },
  "cron": null,
  "enabled": true,
  ...
  "once": false,
  "registered_at": 1715380000000,
  "state": { "repo": "auth-svc", "pr_id": 2401, "self_user": "devuser@example.com", "lastCommentId": 0 }
}
```
...
#### 8.8.4 Cold (TS) — Incidents incident watcher (mockup 03)

`triggers.json` entry:
```json
{
  "id": "incidents-incident-watcher",
  "description": "Pick up new P0/P1 incidents from Incidents.",
  "cron": "*/2 * * * *",
  "command": "tsx $CONDUCTOR_PROJECT_DIR/.conductor/triggers/incidents-incident-watcher.ts"
}
```

`.conductor/triggers/incidents-incident-watcher.ts`:
```ts
#!/usr/bin/env tsx
import { conductor, incidents } from '@conductor/sdk';

const state = JSON.parse(process.env.CONDUCTOR_TRIGGER_STATE ?? '{"lastCheckedAt":0}');

const incidents = await incidents.list_incidents({
  severity: ['P0', 'P1'],
  state: 'active',
...
for (const inc of incidents) {
  await conductor.recipe.run({
    ...
    attach_to_inbox_item_id: `incidents:${inc.id}`,
  });
}
...
| LogSearch error-log table (mockup 03) | LogSearch MCP server returns rows; agent calls `view.emit` to render |
...
|---|---|---|
| **S1 — LargeService** | DB migrations runner, six-table schema, `better-sqlite3` wrapper, `schema_version` & `settings_kv`, smoke tests | `npm run typecheck` + `npm test` green; CRUD on every table |
...
| **S12 — Incident + Incidents source** | Incidents receiver, multi-source correlation recipe, sparkline view | mockup 03 reproduces |
