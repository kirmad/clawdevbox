...
- `C:\git\clawdevbox-plugins\{calls,logsearch,incidents,metrics}/plugin.yaml` → `.claude-plugin/plugin.json`.
...
### Task 4.3: Migrate `C:\git\clawdevbox-plugins\{calls,logsearch,incidents,metrics}`
...
git commit -m "chore(calls): migrate to .claude-plugin/plugin.json"
git commit -m "chore(logsearch): migrate to .claude-plugin/plugin.json"
git commit -m "chore(incidents): migrate to .claude-plugin/plugin.json"
...
1. Clones (or junctions) each of the migrated plugins from `C:\git\clawdevbox-plugins\{calls,logsearch,incidents,metrics}` and `C:\git\agency-provider` into a tmp `<globalDir>/plugins/`.
...
5. Asserts the calls plugin registered its skill, recipes, and tools.
6. Asserts the incidents/logsearch/metrics plugins registered their trigger types.
