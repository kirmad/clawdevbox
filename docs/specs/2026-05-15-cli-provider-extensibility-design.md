...
provides:
  agent_clis:
    - id: agency                              # provider id; collision-checked
      ...
      display_name: "Agency"        # shown in init chooser
...
? Which agent CLI should this workspace use by default?
  ❯ GitHub Copilot CLI       (✓ copilot.exe 1.2.3)
    ...
    Agency         (✓ agency.exe 4.5)   ← only when agency-cli plugin installed
...
5. The agent-CLI chooser shows Agency alongside copilot/claude.
...
```json
{
  ...
  "providers": [
    ...
    { "id": "agency", "display_name": "Agency", "description": "...",
