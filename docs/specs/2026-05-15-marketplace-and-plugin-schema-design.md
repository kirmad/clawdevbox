...
```json
{
  ...
  "clawdevbox": {
    ...
    "tools":   [{ "id": "calls.fetch_call", "file": "tools/fetch_call.ts", "runtime": "tsx" }],
    "trigger_types": [{ "id": "calls.audit-watcher", "file": "triggers/audit.ts",
                         "default_cron": "*/10 * * * *",
...
| `C:\git\clawdevbox-plugins\{calls,logsearch,incidents,metrics}/plugin.yaml` | External samples | Migrate to `.claude-plugin/plugin.json`. |
...
- Backward compat: every clawdevbox-plugins sample (calls, logsearch, incidents, metrics) installs cleanly after migration.
