...
Examples in plugin `cfv`:
- `tools/fetch_call.ts` → tool id `cfv.fetch_call`
- `tools/_auth.ts` → private helper, ignored
- `tools/get_flow.py` → tool id `cfv.get_flow`, runtime `python`

When the manifest declares `clawdevbox.tools: [{ "id": "cfv.fetch_call", ... }]` explicitly, the namespacing is the author's responsibility.
...
The four existing sample plugins (`cfv`, `dgrep`, `icm`, `metrics`) all use explicit arrays today after the Phase 4.3 migration. They keep working unchanged because explicit arrays are the third tier of the polymorphic field. No re-migration needed.
