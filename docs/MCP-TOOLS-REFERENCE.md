...
plugin sync via 'copilot' (GitHub Copilot CLI)
  ...
  pull (dry-run): discovered 2 plugin(s), 1 opted in.
    + superpowers@superpowers-marketplace /home/me/.copilot/plugins/superpowers-superpowers-marketplace
      calls@example-plugins /home/me/.copilot/plugins/calls-example-plugins
...
|---|---|---|---|
| `id` | string | ✅ | Caller-chosen unique key. Conventionally `<source>:<kind>:<localId>` (e.g. `ado:pr:2401`, `incidents:incident:482991123`). |
| `kind` | string | ✅ | Category — `pr_review`, `workitem`, `incident`, `epic`, or any caller-defined string. Filters the list. |
| `source` | string | ✅ | Origin system — `ado`, `incidents`, `manual`, plugin id, etc. Used as the fallback push body when no preview/title is set. |
...
A 5-minute final-judge PR review surface. The artifact is produced by the
`pr-walkthrough` recipe (ADO plugin) via the `build-pr-walkthrough` skill,
and rendered by the built-in renderer at
...
{
  "type": "pr-walkthrough",
  "id": "pr-walkthrough-<prId>",
  "title": "<repo> #<prId> — <prTitle>",
  "recipe_instance_id": "<instance>",
  "source": { "kind": "git" | "ado", "repo": "<path>", "commit": "<sha>" }
}
```
...
<workspace>/artifacts/pr-walkthrough-<prId>/
  manifest.json                     ← standard clawdevbox manifest
  walkthrough.json                  ← agent-authored narrative
                                     (verdict, confidence, attention,
                                      FAQ, disqualifiers, summary, steps)
  original__<safe>.txt              ← pre-PR file content per step
  modified__<safe>.txt              ← post-PR file content per step
  diff__<safe>.patch                ← git show -U8 unified diff per step
  qa/step-<N>.json                  ← Q&A thread per step (server-owned)
```

`<safe>` is the file path with `/` and `\` replaced by `__`.

The narrative shape is documented in
`docs/superpowers/specs/2026-06-14-pr-walkthrough-design.md`.

### `pr-walkthrough.answer` MCP tool

Used by agents to reply to a reviewer Q&A question on a PR walkthrough
artifact. Server-side this appends the answer to the artifact's
`qa/step-<N>.json` thread file; the renderer's poll picks it up within 3s.

**Arguments:**
| Field         | Type    | Required | Description                                        |
|---------------|---------|----------|----------------------------------------------------|
| `artifact_id` | string  | yes      | The PR walkthrough artifact id                     |
| `step_n`      | number  | yes      | The step number the question was asked on         |
| `question_id` | string  | yes      | The `q_xxx` id from the dispatched prompt         |
| `text`        | string  | yes      | Agent's answer (markdown OK, ≤16KB)               |

**Errors:**
- Throws if the artifact does not exist
- Throws if `question_id` is not present in the step's thread

### `GET/POST /artifact/<id>/qa/step-<N>.json` HTTP endpoint

Server endpoint backing the renderer's Q&A flow:

| Method | Body                  | Response                                  |
|--------|----------------------|-------------------------------------------|
| GET    | —                    | 200 + `QaEntry[]` (full thread)           |
| POST   | `{ "text": "..." }`  | 201 + the new `QaEntry`                   |

The browser POST is persistence-only; the renderer separately dispatches a
structured prompt to the live agent session via `/spawn` or `/dispatch`
(same routing as `_comment-overlay.mjs`).
