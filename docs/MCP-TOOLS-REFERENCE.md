...
- [Trigger](#trigger) — `trigger.*` (8 tools)
...
Spawn a fresh agent-CLI session running a recipe. Creates (or reuses) a
workspace, writes that workspace's `.mcp.json` so the spawned CLI sees this
MCP server, mints a `RecipeInstance` row, then detach-spawns the CLI inside a
node-pty and returns immediately. See the **Story** section below for the
...
| Name                       | Type                                       | Required | Description                                                                                  |
|----------------------------|--------------------------------------------|----------|----------------------------------------------------------------------------------------------|
| `id`                       | string                                     | yes      | Recipe id to run. Resolved across all scopes.                                                |
...
**Errors:** `INVALID_ID`, `NOT_FOUND` (recipe), `WORKSPACE_NOT_FOUND`,
...
_8 tools — Plugin-declared trigger types and concrete registered instances._
