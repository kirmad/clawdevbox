...
The ADO plugin no longer ships an external MCP server as its primary tool surface. The five tools (`ado.get_pr`, `ado.list_pr_comments`, `ado.comment_pr`, `ado.list_iterations`, `ado.get_pr_status`) are now **hostable tools** under `../tools/` — single-file scripts hosted in-process by the Clawdevbox MCP server (spec §10.3).
