...
// Copy bundled plugins (e.g. dev-buddy) so they're available on fresh
// installs without requiring explicit plugin.install. The runtime
// resolves them via `<dist>/../plugins/<name>/`.
const bundledPlugins = ['dev-buddy'];
const bundledPluginsDst = join(root, 'plugins');
mkdirSync(bundledPluginsDst, { recursive: true });
for (const name of bundledPlugins) {
  const src = join(repoRoot, 'plugins', name);
  if (!existsSync(src)) {
    process.stderr.write(`bundled plugin missing: ${src}\n`);
    continue;
  }
  const dest = join(bundledPluginsDst, name);
  cpSync(src, dest, {
    recursive: true,
    filter: (p) =>
      !/[/\\]node_modules([/\\]|$)/.test(p) &&
...
// Copy the supervisor scripts (Windows auto-restart wrapper). These
// PowerShell files are invoked by install-supervisor.ts at init time
// and at user logon via Task Scheduler — they need to live next to
// the compiled cli.js in production. Locator: install-supervisor.ts
// probes both `<dist>/supervisor/` and `<repo>/mcp-server/supervisor/`.
const supervisorSrc = join(root, 'supervisor');
const supervisorDst = join(outDir, 'supervisor');
if (existsSync(supervisorSrc)) {
  mkdirSync(supervisorDst, { recursive: true });
  cpSync(supervisorSrc, supervisorDst, { recursive: true });
}
...
process.stdout.write(
  `built dist/cli.js (+ ${existsSync(rendererDst) ? 'renderers/' : 'no renderers'}` +
    ...
    `, ${existsSync(supervisorDst) ? 'supervisor/' : 'no supervisor'}` +
    `, ${existsSync(bundledPluginsDst) ? 'plugins/' : 'no plugins'})\n`,
);
