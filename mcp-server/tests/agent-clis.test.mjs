...
for (const row of MATRIX) {
  test(`copilot argv: ${row.mode} × ${row.kind}`, async () => {
    ...
    // --no-ask-user must immediately precede --yolo: --no-ask-user suppresses
    // Copilot's confirmation prompts and --yolo grants autonomous tool
    // permissions; the two flags are a paired unit that must stay adjacent.
    const noAskCount = c.args.filter((a) => a === '--no-ask-user').length;
    assert.equal(noAskCount, 1, `expected exactly one --no-ask-user: ${c.args.join(' ')}`);
    const yoloCount = c.args.filter((a) => a === '--yolo').length;
    assert.equal(yoloCount, 1, `expected exactly one --yolo: ${c.args.join(' ')}`);
...
test('copilot argv: --model <name> appended when opts.model is set', async () => {
  const ws = await makeWs();
  const ctx = captureSpawnCtx(buildProviderCtx(ws, {}));
  const opts = baseOpts({ ws, mode: 'interactive', kind: 'new', sessionId: 's', prompt: undefined });
  opts.model = 'claude-opus-4.7-1m-internal';
  await (await copilotProvider.spawnSession(ctx, opts)).exited;
  const c = ctx._captured();
  const i = c.args.indexOf('--model');
  assert.ok(i >= 0, `copilot must pass --model when opts.model is set; argv: ${c.args.join(' ')}`);
  assert.equal(c.args[i + 1], 'claude-opus-4.7-1m-internal');
});

// Session-quality defaults: every clawdevbox-spawned copilot session gets
// --model claude-opus-4.8 --context long_context --effort max UNLESS the
// caller overrides them. (Previously this test asserted --model was omitted
// when unset; the new contract is a REQUIRED default, not omission.)
test('copilot argv: quality defaults (model/context/effort) applied when unset', async () => {
  const ws = await makeWs();
  const ctx = captureSpawnCtx(buildProviderCtx(ws, {}));
  await (await copilotProvider.spawnSession(
    ctx,
  ...
  const mIdx = c.args.indexOf('--model');
  assert.ok(mIdx >= 0, `copilot must default --model; argv: ${c.args.join(' ')}`);
  assert.equal(c.args[mIdx + 1], 'claude-opus-4.8');

  const ctxIdx = c.args.indexOf('--context');
  assert.ok(ctxIdx >= 0, `copilot must default --context; argv: ${c.args.join(' ')}`);
  assert.equal(c.args[ctxIdx + 1], 'long_context');

  const effIdx = c.args.indexOf('--effort');
  assert.ok(effIdx >= 0, `copilot must default --effort; argv: ${c.args.join(' ')}`);
  assert.equal(c.args[effIdx + 1], 'max');

  // Each quality flag must be emitted EXACTLY once.
  assert.equal(c.args.filter((a) => a === '--model').length, 1, `one --model: ${c.args.join(' ')}`);
  assert.equal(c.args.filter((a) => a === '--context').length, 1, `one --context: ${c.args.join(' ')}`);
  assert.equal(c.args.filter((a) => a === '--effort').length, 1, `one --effort: ${c.args.join(' ')}`);
});

test('copilot argv: explicit model/context/effort overrides win over defaults', async () => {
  const ws = await makeWs();
  const ctx = captureSpawnCtx(buildProviderCtx(ws, {}));
  const opts = baseOpts({ ws, mode: 'interactive', kind: 'new', sessionId: 's', prompt: undefined });
  opts.model = 'gpt-5.2';
  opts.contextTier = 'default';
  opts.reasoningEffort = 'low';
  await (await copilotProvider.spawnSession(ctx, opts)).exited;
  const c = ctx._captured();

  const mIdx = c.args.indexOf('--model');
  assert.equal(c.args[mIdx + 1], 'gpt-5.2', `explicit model must win; argv: ${c.args.join(' ')}`);
  const ctxIdx = c.args.indexOf('--context');
  assert.equal(c.args[ctxIdx + 1], 'default', `explicit context must win; argv: ${c.args.join(' ')}`);
  const effIdx = c.args.indexOf('--effort');
  assert.equal(c.args[effIdx + 1], 'low', `explicit effort must win; argv: ${c.args.join(' ')}`);

  // Still exactly one of each (no duplicate default + override pair).
  assert.equal(c.args.filter((a) => a === '--model').length, 1, `one --model: ${c.args.join(' ')}`);
  assert.equal(c.args.filter((a) => a === '--context').length, 1, `one --context: ${c.args.join(' ')}`);
  assert.equal(c.args.filter((a) => a === '--effort').length, 1, `one --effort: ${c.args.join(' ')}`);
});

test('copilot argv: --no-ask-user still immediately precedes --yolo with quality flags present', async () => {
  const ws = await makeWs();
  const ctx = captureSpawnCtx(buildProviderCtx(ws, {}));
  await (await copilotProvider.spawnSession(
    ctx,
  ...
  const c = ctx._captured();
  const noAskIdx = c.args.indexOf('--no-ask-user');
  assert.equal(c.args[noAskIdx + 1], '--yolo',
    `--no-ask-user must immediately precede --yolo; argv: ${c.args.join(' ')}`);
});
...
test('claude argv: --model <name> appended when opts.model is set', async () => {
  const ws = await makeWs();
  const ctx = captureSpawnCtx(buildProviderCtx(ws, {}));
  const prevClaude = process.env.CLAWDEVBOX_CLAUDE_PATH;
  delete process.env.CLAWDEVBOX_CLAUDE_PATH;
  try {
    ...
    const c = ctx._captured();
    const i = c.args.indexOf('--model');
    assert.ok(i >= 0, `claude must pass --model when opts.model is set; argv: ${c.args.join(' ')}`);
    assert.equal(c.args[i + 1], 'opus');
  } finally {
    if (prevClaude !== undefined) process.env.CLAWDEVBOX_CLAUDE_PATH = prevClaude;
  }
});

// Session-quality context/effort are Copilot/Agency-only. Claude must ignore
// them entirely (no --context / --effort flags, and no default model injection).
test('claude argv: ignores contextTier/reasoningEffort and injects no quality defaults', async () => {
  const ws = await makeWs();
  const ctx = captureSpawnCtx(buildProviderCtx(ws, {}));
  const prevClaude = process.env.CLAWDEVBOX_CLAUDE_PATH;
  delete process.env.CLAWDEVBOX_CLAUDE_PATH;
  try {
    const opts = baseOpts({ ws, mode: 'interactive', kind: 'new', sessionId: 's', prompt: undefined });
    opts.contextTier = 'long_context';
    opts.reasoningEffort = 'max';
    await (await claudeProvider.spawnSession(ctx, opts)).exited;
    const c = ctx._captured();
    assert.ok(!c.args.includes('--context'), `claude must not emit --context; argv: ${c.args.join(' ')}`);
    assert.ok(!c.args.includes('--effort'), `claude must not emit --effort; argv: ${c.args.join(' ')}`);
    assert.ok(!c.args.includes('--model'), `claude must not default --model; argv: ${c.args.join(' ')}`);
  } finally {
    if (prevClaude !== undefined) process.env.CLAWDEVBOX_CLAUDE_PATH = prevClaude;
  }
});
