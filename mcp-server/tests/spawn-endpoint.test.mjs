...
test('POST /spawn — body.context_tier + reasoning_effort flow through to runRecipe', async () => {
  const db = openDb();
  const ws = makeWs(freshDirs('quality'));
  const stub = makeRunRecipeStub();
  const dispatcher = new Dispatcher(db, ws, { maxConcurrent: 1, runRecipeFn: stub.fn });
  const { server, port } = await startServer(makeCtx(db, dispatcher, ws));
  try {
    ...
    assert.equal(r.status, 200);
    assert.equal(stub.calls.length, 1);
    assert.equal(stub.calls[0].contextTier, 'long_context',
      'context_tier must flow from /spawn body → spawnFromCallback → runRecipe');
    assert.equal(stub.calls[0].reasoningEffort, 'max',
      'reasoning_effort must flow from /spawn body → spawnFromCallback → runRecipe');
  } finally {
    await stopServer(server);
    db.close();
  }
...
test('POST /spawn — invalid context_tier returns 400 and does not spawn', async () => {
  const db = openDb();
  const ws = makeWs(freshDirs('badctx'));
  const stub = makeRunRecipeStub();
  const dispatcher = new Dispatcher(db, ws, { maxConcurrent: 1, runRecipeFn: stub.fn });
  const { server, port } = await startServer(makeCtx(db, dispatcher, ws));
  try {
    const r = await fetch(`http://127.0.0.1:${port}/spawn`, {
      ...
      body: JSON.stringify({
        prompt: 'go',
        provider: 'copilot',
        workspace_path: 'C:/ws/badctx',
        context_tier: 'gigantic',
      }),
    });
    assert.equal(r.status, 400);
    assert.equal(stub.calls.length, 0, 'invalid enum must reject before spawning');
  } finally {
    await stopServer(server);
    db.close();
  }
...
test('POST /spawn — invalid reasoning_effort returns 400 and does not spawn', async () => {
  const db = openDb();
  const ws = makeWs(freshDirs('badeff'));
  const stub = makeRunRecipeStub();
  const dispatcher = new Dispatcher(db, ws, { maxConcurrent: 1, runRecipeFn: stub.fn });
  const { server, port } = await startServer(makeCtx(db, dispatcher, ws));
  try {
