...
test('decomposeDisplayPath splits project/type/rest', () => {
  assert.deepEqual(
    decomposeDisplayPath('clawdevbox/memories/2026-06-07-x.md'),
    { project: 'clawdevbox', type: 'memory', rest: '2026-06-07-x.md' },
  );
  assert.deepEqual(
    decomposeDisplayPath('clawdevbox/wiki/architecture/data-flow.md'),
    { project: 'clawdevbox', type: 'wiki', rest: 'architecture/data-flow.md' },
  );
  assert.equal(decomposeDisplayPath('bare.md'), null);
  assert.equal(decomposeDisplayPath('proj/unknown-folder/file.md'), null);
});
...
test('registerProjectContexts adds contexts for existing project/type subtrees', async () => {
  ...
  try {
    ...
    assert.ok(v1.some((c) => c.path === '/clawdevbox/memories'),
      'expected context for /clawdevbox/memories');
