...
test('reclaimOrphanFires moves a crashed running fire to retrying with service_restart_orphan', () => {
  const db = open();
  const ws = ensureWorkspace(db, { path: 'C:\\tmp\\orphan1' });
  const trg = mkTrigger(db, ws.id, 'orphan.trg#1');
  const fire = enqueueFire(db, { workspace_id: ws.id, source: 'cron', trigger_id: trg });
  // Simulate a previous process that claimed the fire (status=running) then crashed.
  claimNextFire(db);
  assert.equal(getFire(db, fire.fire_id)?.status, 'running');

  const result = reclaimOrphanFires(db);

  assert.deepEqual(result.reclaimed, [fire.fire_id]);
  assert.deepEqual(result.dead, []);
  const row = getFire(db, fire.fire_id);
  assert.equal(row?.status, 'retrying');
  assert.equal(row?.error, 'service_restart_orphan');
  assert.equal(row?.attempt, 2);
...
test('reclaimOrphanFires unblocks a queued fire stuck behind a crashed running fire on the same trigger', () => {
  const db = open();
  const ws = ensureWorkspace(db, { path: 'C:\\tmp\\orphan2' });
  const trg = mkTrigger(db, ws.id, 'orphan.trg#2');
  // Oldest fire was claimed (running) by a now-dead process.
  const stuck = enqueueFire(db, { workspace_id: ws.id, source: 'cron', trigger_id: trg, scheduled_at: 1 });
  claimNextFire(db);
  // A newer fire queued behind it while the process was down.
  const next = enqueueFire(db, { workspace_id: ws.id, source: 'cron', trigger_id: trg, scheduled_at: 2 });

  // Before reclaim: overlap-skip blocks the queued fire (the running orphan
  // makes claimNextFire skip `next`), so nothing claimable → queue drains to skipped.
  // Reclaim first, THEN the queued fire must be claimable.
  reclaimOrphanFires(db);
  assert.notEqual(getFire(db, stuck.fire_id)?.status, 'running');

  const claimed = claimNextFire(db);
  assert.equal(claimed?.fire_id, next.fire_id);
  assert.equal(claimed?.status, 'running');
  db.close();
});

test('reclaimOrphanFires marks an attempt-exhausted orphan dead', () => {
  const db = open();
  const ws = ensureWorkspace(db, { path: 'C:\\tmp\\orphan3' });
  const trg = mkTrigger(db, ws.id, 'orphan.trg#3');
  const fire = enqueueFire(db, { workspace_id: ws.id, source: 'cron', trigger_id: trg, max_attempts: 1 });
  claimNextFire(db);

  const result = reclaimOrphanFires(db);

  assert.deepEqual(result.dead, [fire.fire_id]);
...
test('reclaimOrphanFires preserves genuinely active in-process fires', () => {
  const db = open();
  const ws = ensureWorkspace(db, { path: 'C:\\tmp\\orphan4' });
  const trg = mkTrigger(db, ws.id, 'orphan.trg#4');
  const alive = enqueueFire(db, { workspace_id: ws.id, source: 'cron', trigger_id: trg });
  claimNextFire(db);

  // This process still owns `alive` (it is in the dispatcher in-flight set).
  const result = reclaimOrphanFires(db, { activeFireIds: [alive.fire_id] });

  assert.deepEqual(result.reclaimed, []);
  assert.deepEqual(result.dead, []);
  assert.equal(getFire(db, alive.fire_id)?.status, 'running');
  db.close();
});

// -------------------------------------------- orphan reclaim → dead-letter

function deadInbox(db) {
  return db
    .prepare(
      `SELECT id, workspace_id, title, preview, source, status, trigger_id, fire_id
         FROM inbox_items WHERE source = 'trigger-dead'`,
    )
    .all();
}

test('reclaimOrphanFires dead-letters an exhausted orphan: one deterministic inbox item + trigger last_run_status/error', () => {
  const db = open();
  const ws = ensureWorkspace(db, { path: wsPath('orphan-dl-1') });
  const trg = mkTrigger(db, ws.id, 'orphan.dl#1');
  const fire = enqueueFire(db, { workspace_id: ws.id, source: 'cron', trigger_id: trg, max_attempts: 1 });
  claimNextFire(db);
  ...
  const inbox = deadInbox(db);
  assert.equal(inbox.length, 1, 'exactly one dead-letter inbox item');
  const item = inbox[0];
  assert.equal(item.id, `inb_dead_${fire.fire_id}`, 'deterministic inbox id keyed on fire_id');
  assert.equal(item.fire_id, fire.fire_id);
  assert.equal(item.trigger_id, trg, 'linked to the existing trigger');
  assert.equal(item.workspace_id, ws.id);
  assert.equal(item.status, 'unread');
  // Distinguish interruption reason: this is a service_restart orphan, not a
  // failure of the trigger logic. Title/preview must say so.
  assert.ok(item.title.includes(trg), 'title references the trigger id');
  assert.ok(
    /restart|orphan|interrupt/i.test(`${item.title} ${item.preview}`),
    'title/preview attributes the dead-letter to the service restart',
  );
  assert.ok(
    !/failed permanently/i.test(item.title),
    'must not claim the trigger logic itself failed permanently',
  );

  const t = db
    .prepare(`SELECT last_run_status, last_run_error FROM triggers WHERE id=?`)
    .get(trg);
  assert.equal(t.last_run_status, 'error');
  assert.equal(t.last_run_error, 'service_restart_orphan');
  db.close();
});

test('reclaimOrphanFires dead-letters an exhausted orphan with no trigger without an FK break', () => {
  const db = open();
  const ws = ensureWorkspace(db, { path: wsPath('orphan-dl-2') });
  // No trigger_id → source manual, single attempt so reclaim goes straight to dead.
  const fire = enqueueFire(db, { workspace_id: ws.id, source: 'manual', max_attempts: 1 });
  claimNextFire(db);
  ...
  const inbox = deadInbox(db);
  assert.equal(inbox.length, 1, 'dead-letter item still created for a trigger-less fire');
  assert.equal(inbox[0].fire_id, fire.fire_id, 'inbox linked to the fire');
  assert.equal(inbox[0].trigger_id, null, 'no trigger link → null, no FK break');
  db.close();
});

test('reclaimOrphanFires dead-letter is idempotent: re-running creates no duplicate inbox item', () => {
  const db = open();
  const ws = ensureWorkspace(db, { path: wsPath('orphan-dl-3') });
  const trg = mkTrigger(db, ws.id, 'orphan.dl#3');
  const fire = enqueueFire(db, { workspace_id: ws.id, source: 'cron', trigger_id: trg, max_attempts: 1 });
  claimNextFire(db);

  reclaimOrphanFires(db);
  // Simulate a second crash/restart that re-orphans the same fire row.
  db.prepare(`UPDATE fires SET status='running' WHERE fire_id=?`).run(fire.fire_id);
  reclaimOrphanFires(db);

  assert.equal(deadInbox(db).length, 1, 'deterministic id dedupes across reclaims');
  db.close();
});
