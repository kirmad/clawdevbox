 ...
 * confidence × vote boost) at personal + team scopes, auto-filtered to
 * the active project + _general.
 *
...
test('get_lessons returns empty when no lessons exist', async () => {
  ...
  try {
    const r = await handleGetLessons(ctx, { project: 'clawdevbox' });
    assert.equal(r.personal.length, 0);
    assert.equal(r.team.length, 0);
    assert.equal(r.context.project, 'clawdevbox');
    assert.deepEqual(r.context.projects_searched, ['clawdevbox', '_general']);
...
test('get_lessons separates personal vs team lessons by vault kind', async () => {
  ...
  try {
    await handleAddLesson(ctx, {
      ...
      scope: 'personal', project: 'clawdevbox', confidence: 0.7,
    });
    await handleAddLesson(ctx, {
      content: 'Team rule: TS strict mode is on; never disable',
      scope: 'team', project: 'clawdevbox', confidence: 0.8,
    });
...
test('get_lessons filters by project (active project + _general only)', async () => {
  const { chain, cleanup } = makeVaultChain();
  const ctx = makeCtx(chain);
  try {
    // Lessons across three projects: one matching, one _general, one unrelated.
    await handleAddLesson(ctx, {
      content: 'clawdevbox lesson', scope: 'personal',
      project: 'clawdevbox', confidence: 0.8,
    });
    await handleAddLesson(ctx, {
      content: 'general lesson', scope: 'personal',
      project: '_general', confidence: 0.7,
    });
    await handleAddLesson(ctx, {
      content: 'unrelated repo lesson', scope: 'personal',
      project: 'someother', confidence: 0.9,
    });

    const r = await handleGetLessons(ctx, { project: 'clawdevbox' });
    assert.equal(r.personal.length, 2, `expected 2 lessons (clawdevbox + _general); got: ${r.personal.map(l => l.project).join(', ')}`);
    const projects = r.personal.map((l) => l.project).sort();
    assert.deepEqual(projects, ['_general', 'clawdevbox']);
    assert.equal(r.context.total_candidates, 2);
  } finally {
    await cleanup();
    cleanupCtx(ctx);
  ...
  try {
    for (let i = 0; i < 5; i++) {
      ...
      await handleAddLesson(ctx, {
        ...
        scope: 'personal', project: 'clawdevbox', confidence: 0.5 + i * 0.05,
      });
    }
    for (let i = 0; i < 5; i++) {
      await handleAddLesson(ctx, {
        content: `Team lesson number ${i} with some unique trailing text ${i}`,
        scope: 'team', project: 'clawdevbox', confidence: 0.5 + i * 0.05,
      });
    }

    const r = await handleGetLessons(ctx, {
      project: 'clawdevbox', limit_personal: 2, limit_team: 3,
    });
    assert.equal(r.personal.length, 2);
...
test('get_lessons confidence is decay-adjusted (older lesson scores lower)', async () => {
  ...
  try {
    ...
    ctxNow.chain = chain;
    try {
      const r = await handleGetLessons(ctxNow, { project: 'clawdevbox' });
      assert.equal(r.personal.length, 1);
...
test('get_lessons defaults project from env var when not passed', async () => {
  const { chain, cleanup } = makeVaultChain();
  const ctx = makeCtx(chain);
  const prevEnv = process.env.CLAWDEVBOX_PROJECT;
  process.env.CLAWDEVBOX_PROJECT = 'myproj';
  try {
    await handleAddLesson(ctx, {
      content: 'Project-specific lesson', scope: 'personal',
      project: 'myproj', confidence: 0.8,
    });
    const r = await handleGetLessons(ctx, {});
    assert.equal(r.context.project, 'myproj');
    assert.equal(r.personal.length, 1);
  } finally {
    if (prevEnv === undefined) delete process.env.CLAWDEVBOX_PROJECT;
    else process.env.CLAWDEVBOX_PROJECT = prevEnv;
    await cleanup();
    cleanupCtx(ctx);
  }
});
...
test('get_lessons ignores non-lesson files in lessons/ directory', async () => {
  ...
  try {
    ...
    const wrongDir = join(chain[0].path, 'memories', 'clawdevbox', 'lessons');
    ...
    await handleAddLesson(ctx, {
      ...
      project: 'clawdevbox', confidence: 0.7,
    });
