...
import {
  handleAddFact, handleAddLesson, handleAddSessionSummary,
  handleAddWikiPage, handleGetFact, handleMemoryStatus,
...
test('handleAddFact writes file, sidecar event, and commits', async () => {
  const { chain, cleanup } = makeVaultChain();
  const ctx = makeCtx(chain);
  try {
    const result = await handleAddFact(ctx, {
    ...
    const filePath = join(teamRoot, 'memories', 'facts', result.slug);
    ...
    const eventsFile = join(teamRoot, 'memories', 'facts', '.events',
    ...
    assert.match(log, /fact: Always validate JWT exp/);

    // Round-trip via handleGetFact
    const got = await handleGetFact(ctx, {
      path: result.path,
      scope: 'team',
    });
    assert.equal(got.type, 'fact');
...
test('handleAddFact errors when no vault matches scope', async () => {
  const { chain, cleanup } = makeVaultChain();
...
test('handleAddLesson writes to lessons/ with initial_confidence', async () => {
  ...
  try {
    ...
    const filePath = join(chain[0].path, 'memories', 'lessons', result.slug);
...
test('handleAddSessionSummary uses minute granularity in filename', async () => {
  ...
  try {
    ...
    const md = readFileSync(join(chain[0].path, 'memories', 'sessions', result.slug), 'utf8');
...
test('handleAddWikiPage creates nested path with wikilink-friendly body', async () => {
  ...
  try {
    ...
    const filePath = join(chain[1].path, 'memories', 'wiki', 'architecture', 'data-flow.md');
    assert.ok(existsSync(filePath));
    const md = readFileSync(filePath, 'utf8');
...
// ---------------------------------------------------------------------------
// get_fact across vaults
// ---------------------------------------------------------------------------

test('handleGetFact finds file in correct vault when scope omitted', async () => {
  const { chain, cleanup } = makeVaultChain();
  const ctx = makeCtx(chain);
  try {
    const created = await handleAddFact(ctx, {
      content: 'unique-content', scope: 'personal',
      citations: 'x', reason: 'because.',
    });
    const got = await handleGetFact(ctx, { path: created.path });
...
test('handleGetFact throws when file not found', async () => {
  const { chain, cleanup } = makeVaultChain();
  const ctx = makeCtx(chain);
  try {
    await assert.rejects(
      () => handleGetFact(ctx, { path: 'nonexistent/facts/nope.md' }),
      /not found/i,
    );
  } finally {
    await cleanup();
...
test('handleMemoryInit scaffolds folders and registers qmd collections idempotently', async () => {
  ...
  try {
    ...
    assert.ok(existsSync(join(chain[0].path, 'memories', 'facts')));
    assert.ok(existsSync(join(chain[0].path, 'memories', 'lessons')));
    assert.ok(existsSync(join(chain[0].path, 'memories', 'sessions')));
    assert.ok(existsSync(join(chain[0].path, 'memories', 'wiki')));
    assert.ok(existsSync(join(chain[1].path, 'memories', 'facts')));
...
test('handleSearchMemory filters by types', async () => {
  const { chain, cleanup } = makeVaultChain();
  const ctx = makeCtx(chain);
  try {
    await handleMemoryInit(ctx, {});
    ...
    const memOnly = await handleSearchMemory(ctx, { query: 'foobar', types: ['fact'] });
    assert.ok(memOnly.results.every((r) => r.type === 'fact'));
  } finally {
    await cleanup();
    cleanupCtx(ctx);
  }
