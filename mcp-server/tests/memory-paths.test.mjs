...
test('buildFilename uses date prefix for fact/lesson, datetime for session', () => {
  const created = new Date('2026-06-07T10:38:15Z');
  assert.equal(buildFilename('fact', 'JWT validation', created), '2026-06-07-jwt-validation.md');
...
test('buildFilename falls back to "untitled" when slug empty', () => {
  const created = new Date('2026-06-07T10:38:15Z');
  assert.equal(buildFilename('fact', '!!!', created), '2026-06-07-untitled.md');
});
...
test('vaultPathFor builds the correct file path under the memories/ subroot', () => {
  ...
  assert.equal(
    vaultPathFor(vault, 'fact', '2026-06-07-jwt.md').replace(/\\/g, '/'),
    '/v/memories/facts/2026-06-07-jwt.md',
  );
});
...
test('eventsPathFor maps to sibling .events folder for flat types', () => {
  const vault = { id: 'x', path: '/v', kind: 'personal', remote: null };
  assert.equal(
    eventsPathFor(vault, 'fact', '2026-06-07-jwt.md').replace(/\\/g, '/'),
    '/v/memories/facts/.events/2026-06-07-jwt.jsonl',
  );
});
...
test('eventsPathFor handles nested wiki paths', () => {
  const vault = { id: 'x', path: '/v', kind: 'personal', remote: null };
  assert.equal(
    eventsPathFor(vault, 'wiki', 'architecture/data-flow.md').replace(/\\/g, '/'),
    '/v/memories/wiki/.events/architecture/data-flow.jsonl',
  );
});
