...
import {
  handleAddFact, handleAddLesson, handleAddWikiPage, handleGetFact,
  handleVoteFact, handleVoteLesson, handleVoteWiki,
  handleUpdateWiki,
...
test('vote_fact appends event, commits, returns vote tally', async () => {
  const { chain, cleanup } = makeVaultChain();
  const ctx = makeCtx(chain);
  try {
    const mem = await handleAddFact(ctx, {
      content: 'Memory content', scope: 'team',
      citations: 'a', reason: 'b b',
    });
    const voted = await handleVoteFact(ctx, {
    ...
    const got = await handleGetFact(ctx, { path: mem.path, scope: 'team' });
    ...
    assert.match(log, /vote: up memories\/facts/);
  } finally {
    await cleanup();
    cleanupCtx(ctx);
  }
...
test('vote_fact rejects when target is a different type', async () => {
  const { chain, cleanup } = makeVaultChain();
  const ctx = makeCtx(chain);
  try {
    const lesson = await handleAddLesson(ctx, {
      content: 'lesson content', scope: 'personal', confidence: 0.5,
    });
    await assert.rejects(
      () => handleVoteFact(ctx, { path: lesson.path, scope: 'personal', direction: 'up' }),
      /vote_fact called on a lesson document/i,
    );
  } finally {
    await cleanup();
    cleanupCtx(ctx);
...
test('vote_lesson returns decay-adjusted confidence', async () => {
  ...
  try {
    const lesson = await handleAddLesson(ctx, {
      content: 'L1 content unique', scope: 'personal', confidence: 0.6,
    });
...
test('vote_lesson per-actor latest-wins (flip up→down)', async () => {
  ...
  try {
    const lesson = await handleAddLesson(ctx, {
      content: 'unique lesson', scope: 'personal',
    });
...
test('add_lesson with exact-duplicate content appends reinforced event', async () => {
  ...
  try {
    ...
    const got = await handleGetFact(ctx, { path: first.path, scope: 'personal' });
...
test('update_wiki append adds content to end of body', async () => {
  ...
  try {
    ...
    const result = await handleUpdateWiki(ctx, {
      path: 'wiki/guide.md', scope: 'team',
      operation: 'append', content: '\n## New section\n\nAppended.\n',
    });
    assert.equal(result.action, 'updated');
    assert.equal(result.operation, 'append');

    const got = await handleGetFact(ctx, { path: 'wiki/guide.md', scope: 'team' });
...
test('update_wiki prepend adds content to start of body', async () => {
  ...
  try {
    ...
    await handleUpdateWiki(ctx, {
      path: 'wiki/g.md', scope: 'team',
      operation: 'prepend', content: '> [!note] Prepended\n',
    });
    const got = await handleGetFact(ctx, { path: 'wiki/g.md', scope: 'team' });
