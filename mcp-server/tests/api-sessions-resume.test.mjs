...
test('POST /api/sessions/<id>/resume — inline session falls back to defaultAgentCli (no 422)', async () => {
  const db = openDb();
  const ws = makeWs();
  insertWorkspace(db, 'wsI', 'C:/wsI');
  insertRecipeInstance(db, 'ri_inline', 'implement-work-item', 'wsI');
  // 'inline' is NOT a registered provider — the recipe ran inline in the
  // calling agent. Resume must fall back to cfg.defaultAgentCli ('copilot')
  // rather than 422 "provider not registered".
  insertArchivedSession(db, {
    id: 'as_inline', workspaceId: 'wsI', recipeInstanceId: 'ri_inline',
    cliSessionId: 'sess_inline_abc', agentCli: 'inline',
  });
  const dispatcher = new Dispatcher(db, ws, { maxConcurrent: 1 });
  const stub = makeStubRunRecipe();
  const { server, port } = await startServer(
    makeCtx(db, dispatcher, ws, { runRecipeFn: stub.fn, cfg: { defaultAgentCli: 'copilot' } }),
  );
  try {
    const r = await fetch(`http://127.0.0.1:${port}/api/sessions/ri_inline/resume`, { method: 'POST' });
    const body = await r.json();
    assert.equal(r.status, 200, JSON.stringify(body));
    assert.equal(body.ok, true);
    // runRecipe was invoked with the fallback CLI, not 'inline'.
    assert.equal(stub.calls.length, 1);
    assert.equal(stub.calls[0].agentCli, 'copilot');
    assert.equal(stub.calls[0].resumeOf, 'sess_inline_abc');
  } finally {
    await stopServer(server);
    db.close();
  }
