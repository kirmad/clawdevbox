...
// ---------------------------------------------------------------------------
// Test B — trigger.fire → dispatcher → recipe spawn.
// ---------------------------------------------------------------------------

const SAVED_RECIPE_YAML = [
  'id: e2e-test-recipe',
  'name: E2E Test Recipe',
  'description: Triggered recipe driven by the e2e-test-runner agent CLI.',
  'agent_cli: e2e-test-runner',
  'steps: []',
  '',
].join('\n');
...
test(
  'real e2e: trigger.fire → dispatcher → recipe-bound spawn drives agent to recipe.done',
  { timeout: 120_000 },
  async () => {
    const svc = await spawnKernelService({ projectConfig: { default_agent_cli: 'e2e-test-runner' } });

    // Pre-register the project dir as a workspace in BOTH the registry
    // (workspaces/index.json) and the kernel DB so that:
    //   (a) trigger.fire's ensureWorkspace finds the existing DB row
    //       (matched by path) and reuses its id, and
    //   (b) recipe.done's context resolver finds the same id in the
    //       registry via the X-Clawdevbox-Workspace-Id header.
    //
    // Must run BEFORE any MCP tool that touches the DB (trigger.register,
    // recipe.upsert, etc. all call ensureWorkspace transitively and would
    // otherwise mint a fresh id that's never written to the registry).
    const projectWsId = 'ws_e2e_project';
    const workspacesRoot = join(svc.globalDir, 'workspaces');
    mkdirSync(workspacesRoot, { recursive: true });
    const resolvedProjectDir = resolve(svc.projectDir);
    writeFileSync(
      join(workspacesRoot, 'index.json'),
      JSON.stringify(
        {
          workspaces: {
            [projectWsId]: {
    ...
    {
      const wdb = new BetterSqlite3(join(svc.globalDir, 'clawdevbox.db'));
      try {
        wdb.pragma('journal_mode = WAL');
        wdb.pragma('foreign_keys = ON');
        wdb.prepare(
          `INSERT OR IGNORE INTO workspaces (id, path, name, parent_workspace_id, created_at)
           VALUES (?, ?, ?, NULL, ?)`,
        ).run(projectWsId, resolvedProjectDir, 'e2e-project', Date.now());
        const verify = wdb
          .prepare('SELECT id FROM workspaces WHERE path = ?')
          .get(resolvedProjectDir);
        assert.ok(verify, 'pre-seed workspace row exists');
        assert.equal(verify.id, projectWsId, 'pre-seed wins (no prior row)');
      } finally {
        wdb.close();
      }
    }
    ...
    try {
      ...
      // 1. Author the recipe in project scope.
      await client.callTool('recipe.upsert', {
        id: 'e2e-test-recipe',
        scope: 'project',
        source: SAVED_RECIPE_YAML,
        format: 'yaml',
      });

      // 2. Create a trigger TYPE that binds its callback to the recipe.
      //    The script is never invoked for recipe bindings (see
      //    dispatcher.runFire), so it can be a no-op.
      await client.callTool('trigger.create_template', {
        id: 'local.e2e-watcher',
        scope: 'project',
        description: 'E2E watcher trigger TYPE (real-spawn test).',
        runtime: 'node',
        script: "console.log('e2e-watcher trigger script — should not run for recipe bindings');\n",
        binds_callback_to_recipe: 'e2e-test-recipe',
      });

      // 3. Register an instance of the trigger.
      const regResult = await client.callTool('trigger.register', {
        type_id: 'local.e2e-watcher',
        params: {},
        cron: false,
      });
      const regStructured = regResult && regResult.structuredContent;
      assert.ok(regStructured, `trigger.register missing structuredContent: ${JSON.stringify(regResult)}`);
      const triggerId = regStructured.id;
      assert.ok(triggerId, 'trigger id present');

      // 4. Fire the trigger.
      const fireResult = await client.callTool('trigger.fire', { id: triggerId, payload: {} });
      const fireStructured = fireResult && fireResult.structuredContent;
      const fireId = fireStructured && fireStructured.fire_id;
      assert.ok(fireId, `trigger.fire missing fire_id: ${JSON.stringify(fireResult)}`);

      // 5. Poll fires DB until status=success and recipe_instance_id is set.
      const fireRow = await pollUntil(() => {
        const db = openDb(svc.globalDir);
        try {
          const row = db
            .prepare('SELECT fire_id, status, trigger_id, recipe_instance_id FROM fires WHERE fire_id = ?')
            .get(fireId);
          if (row && row.status === 'success' && row.recipe_instance_id) return row;
          return null;
        } finally {
          db.close();
        }
      }, AGENT_POLL_TIMEOUT_MS, `fires row ${fireId} reaches success`);

      assert.equal(fireRow.status, 'success');
      assert.equal(fireRow.trigger_id, triggerId);
      const instanceId = fireRow.recipe_instance_id;
      assert.ok(instanceId, 'fire.recipe_instance_id populated');

      // 6. Poll recipe_instances until terminal.
      const inst = await pollUntil(() => {
        const db = openDb(svc.globalDir);
        try {
          const row = db
            .prepare('SELECT id, status, message, trigger_id, fire_id, workspace_id, workspace_path FROM recipe_instances WHERE id = ?')
            .get(instanceId);
          if (row && row.status && row.status !== 'running') return row;
          return null;
        } finally {
      ...
      // Diagnostic snapshot before the assertion.
      const logPath = join(inst.workspace_path, '.clawdevbox', 'recipe-instances', `${instanceId}.log`);
      const logTail = safeRead(logPath, 4000);

      assert.equal(
        inst.status,
        'success',
        `recipe_instances.status; message=${inst.message}; log tail:\n${logTail}`,
      );
      assert.equal(inst.trigger_id, triggerId, 'recipe_instances.trigger_id linked');
      assert.equal(inst.fire_id, fireId, 'recipe_instances.fire_id linked');

      // Recipe-instance JSON file
      const instanceFilePath = join(inst.workspace_path, '.clawdevbox', 'recipe-instances', `${instanceId}.json`);
      assert.ok(existsSync(instanceFilePath), `instance file ${instanceFilePath} missing`);
      const instanceFile = JSON.parse(readFileSync(instanceFilePath, 'utf8'));
      assert.equal(instanceFile.status, 'success');
      assert.ok(
        typeof instanceFile.message === 'string' && instanceFile.message.includes('E2E_MARKER_DONE'),
        `expected E2E_MARKER_DONE in message; got ${JSON.stringify(instanceFile.message)}`,
      );
      ...
      assert.ok(
        fullLog.includes('E2E_MARKER_EXIT_OK'),
        `pty log should include E2E_MARKER_EXIT_OK; got:\n${fullLog.slice(-1000)}`,
      );
      ...
      const item = items.find((i) => i.id === `e2e:${instanceId}`);
      assert.ok(item, `inbox item e2e:${instanceId} not found; items: ${items.map((i) => i.id).join(',')}`);
      assert.ok(item.title && item.title.includes(instanceId));
