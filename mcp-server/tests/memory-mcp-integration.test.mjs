...
test('full MCP integration: register, init, add×4, search, get, status', async (t) => {
  ...
  try {
    ...
    const expected = [
      ...
      'get_fact', 'memory_status', 'memory_init', 'search_memory', 'get_wiki_index',
      'vote_fact', 'vote_lesson', 'vote_wiki', 'update_wiki', 'memory_sync',
    ];
    ...
    // add_fact (team)
    const mem = await invokeTool('add_fact', {
    ...
    // get_fact round-trip
    const fetched = await invokeTool('get_fact', {
      path: mem.path,
      scope: 'team',
    });
    assert.equal(fetched.type, 'fact');
...
test('Zod validation rejects malformed args via the registered tool', async () => {
  ...
  try {
    ...
    const addMem = getTool('add_fact');
    assert.throws(
      () => addMem.parameters.parse({
        // missing required `content`
        scope: 'team', citations: 'a', reason: 'b b',
      }),
      /content/i,
    );
    assert.throws(
      () => addMem.parameters.parse({
        content: 'x', scope: 'invalid', citations: 'a', reason: 'b',
      }),
      /invalid|enum/i,
    );
  } finally {
