...
export interface RunRecipeOptions {
  ...
  model?: string;
  /**
   * Optional context-window tier forwarded to the provider's spawnSession
   * (`--context <tier>` on copilot/agency; ignored by other providers).
   */
  contextTier?: ContextTier;
  /**
   * Optional reasoning-effort tier forwarded to the provider's spawnSession
   * (`--effort <level>` on copilot/agency; ignored by other providers).
   */
  ...
  mcpSecret?: string;
  /** Extra ambient env merged into the spawn env (last-wins). Used e.g. to hand a verifier its verdict-file path. */
...
export async function runRecipe(opts: RunRecipeOptions): Promise<RunRecipeResult> {
  ...
  // Pull `name` out of the recipe source so the SPA's master-detail
  // rail can show a meaningful label instead of falling back to the
  // opaque `__adhoc_<instance_id>` recipe_id. Best-effort: a parse
  // failure must NOT block the spawn — the recipe was already
  // validated by the caller.
  let recipeName: string | undefined;
  if (recipeSnapshot) {
    try {
      const parsed = parseRecipeSource(recipeSnapshot);
      if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
        const n = (parsed as Record<string, unknown>).name;
        if (typeof n === 'string' && n.trim().length > 0) recipeName = n.trim();
      }
    } catch { /* recipe already validated upstream; ignore */ }
  }
  ...
  const spawnEnv: Record<string, string> = {
    ...
    // Merged last (last-wins) so callers can inject extra ambient env (e.g. a
    // verifier's verdict-file path). Callers MUST NOT override the reserved
    // CLAWDEVBOX_* keys above.
    ...(opts.extraEnv ?? {}),
  };
