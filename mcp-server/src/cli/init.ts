...
import { installSupervisor } from './install-supervisor.ts';
import { answer, loadInitConfigFile, type InitConfigFile } from './init-config-file.ts';
import {
...
async function runInitInner(flags: Flags): Promise<void> {
  ...
  // ---- Optional config file (--config-file <path>) ----------------------
  // Pre-populates answers to every prompt so init can run unattended.
  // Absent fields fall through to the interactive prompt; with
  // --non-interactive missing fields use documented defaults.
  let answers: InitConfigFile = {};
  const configFilePath = str(flags, 'config-file');
  if (configFilePath) {
    try {
      answers = await loadInitConfigFile(configFilePath);
      log.info(`Loaded answers from ${configFilePath}`);
    } catch (err) {
      log.error(err instanceof Error ? err.message : String(err));
      cancel('init aborted — config file invalid');
      process.exit(1);
    }
  }
  const nonInteractive =
    boolFlag(flags, 'non-interactive') || (configFilePath !== undefined);
  ...
  const answerProjectDir = projectDirFlag ?? answers.project_dir;
  if (installScope === 'project') {
    const projectDirRaw = answerProjectDir ?? abortIfCancel(
      await text({
    ...
    if (!existsSync(projectDir)) {
      const create = nonInteractive ? true : abortIfCancel(
        await confirm({
    ...
    projectDir = answerProjectDir
      ? isAbsolute(answerProjectDir)
        ? resolve(answerProjectDir)
        : resolve(cwd, answerProjectDir)
      : cwd;
  ...
  const globalDirRaw = answers.global_dir ?? abortIfCancel(
    await text({
  ...
  if (existing) {
    const overwrite = answers.overwrite_existing ?? abortIfCancel(
      await confirm({
  ...
  const port = answers.http?.port ?? Number.parseInt(
    abortIfCancel(
      await text({
  ...
  // Token: PRESERVE/GENERATE sentinels OR a literal token string OR
  // an interactive yes/no prompt that mints a fresh one.
  let token: string;
  const tokenAnswer = answers.http?.token;
  if (tokenAnswer === 'PRESERVE' && existing?.http?.token) {
    token = existing.http.token;
  } else if (tokenAnswer === 'GENERATE' || tokenAnswer == null) {
    if (tokenAnswer == null && !nonInteractive) {
      ...
      token = generateNewToken ? generateToken() : (existing?.http?.token ?? generateToken());
    } else {
      token = generateToken();
    }
  } else {
    // Literal token string from the config file
    token = tokenAnswer;
  }

  // ----- Tunnel ---------------------------------------------------------
  const tunnelKind = answers.tunnel?.kind ?? abortIfCancel(
  ...
  if (tunnelKind === 'devtunnel') {
    ...
    const answerTunnelName =
      (answers.tunnel && 'name' in answers.tunnel ? answers.tunnel.name : undefined);
    tunnelName = answerTunnelName ?? abortIfCancel(
      await text({
    ...
    const answerAllowAnon =
      (answers.tunnel && 'allow_anonymous' in answers.tunnel ? answers.tunnel.allow_anonymous : undefined);
    tunnelAllowAnon = answerAllowAnon ?? abortIfCancel(
      await confirm({
        message:
  ...
  const wantNotifications = answers.notifications?.enabled ?? abortIfCancel(
    await confirm({
      message:
  ...
  if (wantNotifications) {
    ...
    if (existingVapid) {
      ...
      const subject = answers.notifications?.subject ?? abortIfCancel(
        await text({
  ...
  try {
    ...
    // ---- External plugins from config file ----------------------------------
    if (answers.external_plugins && answers.external_plugins.length > 0) {
      log.info(`Installing ${answers.external_plugins.length} external plugin(s)...`);
      const pluginsRoot = join(globalDir, 'plugins');
      mkdirSync(pluginsRoot, { recursive: true });
      for (const source of answers.external_plugins) {
        try {
          let pluginDir: string;
          const hashIdx = source.indexOf('#path=');
          if (hashIdx >= 0) {
            const gitUrl = source.slice(0, hashIdx);
            const subPath = source.slice(hashIdx + 6);
            const repoName = gitUrl.replace(/.*\//, '').replace(/\.git$/, '');
            const cloneDir = join(globalDir, 'plugin-sources', repoName);
            if (!existsSync(cloneDir)) {
              const { spawnSync } = await import('node:child_process');
              const r = spawnSync('git', ['clone', '--depth', '1', gitUrl, cloneDir], {
                encoding: 'utf8', timeout: 120_000, windowsHide: true,
              });
              if (r.status !== 0) {
                log.warn(`Failed to clone ${gitUrl}: ${r.stderr?.slice(0, 100)}`);
                continue;
              }
            }
            pluginDir = join(cloneDir, subPath.replace(/^\//, ''));
          } else if (isAbsolute(source)) {
            pluginDir = source;
          } else {
            log.warn(`external_plugins: "${source}" is not an absolute path or git URL — skipping`);
            continue;
          }
          if (!existsSync(pluginDir)) {
            log.warn(`external_plugins: "${pluginDir}" does not exist — skipping`);
            continue;
          }
          // Derive plugin id from the directory name
          const { basename: pathBasename } = await import('node:path');
          const pluginId = pathBasename(pluginDir);
          const dest = join(pluginsRoot, pluginId);
          if (!existsSync(dest)) {
            const { symlinkSync } = await import('node:fs');
            const linkType = process.platform === 'win32' ? 'junction' : 'dir';
            symlinkSync(pluginDir, dest, linkType);
            log.info(`  ✓ ${pluginId} (linked from ${pluginDir})`);
          } else {
            log.info(`  • ${pluginId} already installed`);
          }
        } catch (err) {
          log.warn(`  ✗ ${source}: ${err instanceof Error ? err.message : String(err)}`);
        }
      }
      ensureGlobalNodeModulesLink(globalDir);
    }
    ...
    if (answers.vaults && answers.vaults.length > 0) {
      // Config-file provided vaults — use them directly, clone remotes
      const vaultsDir = join(globalDir, 'vaults');
      for (const v of answers.vaults) {
        const vaultPath = v.path
          ?? (v.remote ? join(vaultsDir, v.id) : join(globalDir, `${v.id}-vault`));
        if (v.remote && !existsSync(vaultPath)) {
          log.info(`Cloning vault "${v.id}" from ${v.remote}...`);
          const { cloneRepo } = await import('./init-vault.ts');
          const cloneArgs = (v as { branch?: string }).branch
            ? ['clone', '--branch', (v as { branch?: string }).branch!, v.remote, vaultPath]
            : undefined;
          let ok: boolean;
          if (cloneArgs) {
            const r = spawnSync('git', cloneArgs, { encoding: 'utf8', windowsHide: true, timeout: 120_000 });
            ok = r.status === 0;
          } else {
            ok = cloneRepo(v.remote, vaultPath);
          }
          if (!ok) {
            log.warn(`Failed to clone vault "${v.id}" — skipping`);
            continue;
          }
        }
        if (!v.path && !existsSync(vaultPath)) {
          const { scaffoldVault } = await import('./init-vault.ts');
          scaffoldVault(vaultPath, { id: v.id, title: v.id, kind: v.kind ?? 'team' });
          const { isGitRepo, initGitRepo } = await import('./init-vault.ts');
          if (!isGitRepo(vaultPath)) initGitRepo(vaultPath);
        }
        vaultEntries.push({
          id: v.id,
          path: vaultPath,
          kind: (v.kind ?? v.scope ?? 'team') as 'personal' | 'team',
          remote: v.remote ?? null,
          ...((v as { branch?: string }).branch ? { branch: (v as { branch?: string }).branch } : {}),
        });
      }
      // Walk includes (parent chain) for each cloned vault
      const { walkParentChain } = await import('./init-vault.ts');
      for (const entry of [...vaultEntries]) {
        if (entry.remote) {
          try {
            const parents = walkParentChain(entry.path, entry.id, entry.remote, vaultsDir);
            vaultEntries.push(...parents);
          } catch { /* ignore chain walk errors */ }
        }
      }
      if (vaultEntries.length > 0) {
        log.info(`Configured ${vaultEntries.length} vault(s) from config file.`);
      }
    } else {
      try {
    ...
    if (installScope === 'global') {
      const installNow = answers.install_service ?? abortIfCancel(
        await confirm({
          message:
    ...
    // ---- Optional: install the auto-restart supervisor (Windows) -------
    // Wraps `clawdevbox start` in a PowerShell loop that respawns on
    // crash + registers a "Run at logon" Task Scheduler task. Default
    // ON for global scope on Windows; off elsewhere (no equivalent on
    // macOS / Linux yet). The config-file `install_supervisor` overrides.
    const isWindows = process.platform === 'win32';
    if (installScope === 'global' && isWindows) {
      const wantSupervisor =
        answers.install_supervisor ?? abortIfCancel(
          await confirm({
            message:
              'Install the auto-restart supervisor? Wraps `clawdevbox start` so it respawns automatically when the daemon crashes, and starts at user logon.',
            initialValue: true,
          }),
        );
      if (wantSupervisor) {
        const supSpinner = spinner();
        supSpinner.start('Registering Clawdevbox Supervisor scheduled task...');
        // Locate repo root: when running from dist, supervisor scripts
        // live next to dist/cli.js → repoRoot = parent of dist. When
        // running from source (tsx), we're at mcp-server/src/cli/ →
        // repoRoot is the clawdevbox checkout (two levels up). The
        // installer will probe both paths.
        const here = dirname(fileURLToPath(import.meta.url));
        const repoRoot = resolve(here, '..', '..', '..');
        const sr = installSupervisor({ repoRoot });
        if (sr.ok) {
          supSpinner.stop('Clawdevbox Supervisor installed (auto-start at logon, restart on crash).');
        } else if (sr.unsupportedPlatform) {
          supSpinner.stop('Supervisor skipped (Windows-only).');
        } else {
          supSpinner.stop(`Supervisor install failed: ${sr.error}`, 1);
          if (sr.output) log.warn(sr.output.trim());
        }
      }
    }

    // ---- Register daemons from init config ---------------------------------
    if (answers.daemons?.length) {
      const regSpinner = spinner();
      regSpinner.start(`Registering ${answers.daemons.length} daemon(s)...`);
      let registered = 0;
      for (const d of answers.daemons) {
        // Resolve command paths relative to plugin dir if plugin is specified
        let resolvedCommand = d.command;
        if (d.plugin) {
          const pluginDir = join(globalDir, 'plugins', d.plugin);
          if (existsSync(pluginDir)) {
            resolvedCommand = d.command.map((arg, i) =>
              i === 0 || arg.startsWith('/') || arg.startsWith('C:') ? arg : join(pluginDir, arg),
            );
          }
        }
        // POST to the running service to register the daemon
        try {
          const body = JSON.stringify({
            ...
            cwd: d.cwd ?? (d.plugin ? join(globalDir, 'plugins', d.plugin) : undefined),
            env: d.env,
            enabled: d.enabled ?? true,
          });
          const resp = await fetch(`http://127.0.0.1:${port}/api/daemons/register`, {
            method: 'POST',
            headers: {
          ...
          if (resp.ok || resp.status === 409) registered++; // 409 = already exists
        } catch { /* service may not be running yet — skip silently */ }
      }
      regSpinner.stop(`Daemons: ${registered}/${answers.daemons.length} registered.`);
    }

    // ---- Register triggers from init config --------------------------------
    if (answers.triggers?.length) {
      const regSpinner = spinner();
      regSpinner.start(`Registering ${answers.triggers.length} trigger(s)...`);
      let registered = 0;
      for (const t of answers.triggers) {
        const triggerId = `${t.type}#${t.key}`;
        try {
          // Call the MCP run_tool endpoint to register the trigger
      ...
      regSpinner.stop(`Triggers: ${registered}/${answers.triggers.length} registered.`);
    }
...
export async function runAgentCliChooser(
  ...
  // Prefer copilot when it's available; otherwise first available; otherwise skip.
  // This makes the chooser's default match the rest of the codebase's `?? 'copilot'`
  // fallback chain (dispatcher.ts:126, main-agent.ts:118, etc.) so a fresh init
  // doesn't silently default to whichever provider happened to register first.
  const defaultProvider =
    detectResults.find((r) => r.provider.id === 'copilot' && r.detect.available)?.provider.id ??
    detectResults.find((r) => r.detect.available)?.provider.id;
