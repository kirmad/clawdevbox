...
async function handleRecipeResume(
  ...
  cfg: ResolvedConfig,
  ws: { projectDir: string; globalDir: string },
): Promise<void> {
  ...
  // Resolve binary + args per agent CLI. Mirrors recipe.ts; resume-only.
  let ptyFile: string;
  let ptyArgs: string[];
  if (agentCli === 'echo-stub') {
    const banner = `[echo-stub resume] session=${sessionId} (was=${recipeInstanceId}) prompt=${JSON.stringify(prompt)}`;
    const artifactId = `${source.recipe_id}-${newInstanceId.slice(3)}-resume`;
    const instancesDirOnDisk = join(workspacePath, '.clawdevbox', 'recipe-instances');
    mkdirSync(instancesDirOnDisk, { recursive: true });
    const scriptPath = join(instancesDirOnDisk, `${newInstanceId}.script.cjs`);
    const scriptBody =
...
const manifest = {
  id: artifactId,
  type: 'markdown',
  title: ${JSON.stringify('Resume: ' + prompt.slice(0, 60))},
...
const body = [
  '# Resume of session ' + process.env.CLAWDEVBOX_SESSION_ID,
  '',
  'This artifact was generated by an echo-stub recipe run **resumed** via the SPA Resume button.',
  '',
  '- session_id: \`' + process.env.CLAWDEVBOX_SESSION_ID + '\`',
  '- recipe_instance_id: \`' + process.env.CLAWDEVBOX_RECIPE_INSTANCE_ID + '\`',
  '- resume_of: \`' + ${JSON.stringify(recipeInstanceId)} + '\`',
  '',
  '## Prompt',
  '',
...
try {
  ...
  inst.message = 'resume complete; artifact ' + artifactId + ' written.';
  inst.steps = [
    { id: 'spawn', title: 'Spawn agent with --resume', status: 'done', started_at: inst.started_at, completed_at: inst.started_at + 50, message: 'session_id=' + process.env.CLAWDEVBOX_SESSION_ID },
    { id: 'write-artifact', title: 'Write artifact', status: 'done', started_at: inst.started_at + 50, completed_at: Date.now(), message: 'wrote ' + artifactId, artifact_id: artifactId },
  ];
  fs.writeFileSync(instPath, JSON.stringify(inst, null, 2));
} catch (err) {
  process.stderr.write(err.message + '\\n');
}
process.stdout.write('[echo-stub resume] wrote ' + artifactId + '\\n');
process.exit(0);
`;
    ...
    // Real GitHub Copilot CLI — resume by id. INTERACTIVE: no -p flag,
    // so the user lands in the running session via the SPA terminal
    // viewer and can chat directly. Keep --allow-all-tools so tool
    // calls don't block on permission prompts.
    ...
    // Same INTERACTIVE pattern for Claude — drop -p so xterm hosts the
    // live conversation.
    if (process.platform === 'win32') {
      ptyFile = 'cmd.exe';
      ptyArgs = ['/d', '/s', '/c', 'claude', '--resume', sessionId];
    } else {
      ptyFile = 'claude';
      ptyArgs = ['--resume', sessionId];
    }
  } else {
    res.writeHead(400, { 'content-type': 'application/json' });
    res.end(JSON.stringify({ error: `unsupported agent_cli: ${agentCli}` }));
    return;
  }
  ...
  spawnEnv.CLAWDEVBOX_PROJECT_DIR = workspacePath;
  spawnEnv.CLAWDEVBOX_RECIPE_INSTANCE_ID = newInstanceId;
  spawnEnv.CLAWDEVBOX_WORKSPACE_ID = source.workspace_id;
  spawnEnv.CLAWDEVBOX_WORKSPACES_ROOT = cfg.workspacesRoot;
  ...
  try {
    ...
    if (cur) writeRecipeInstance(workspacePath, { ...cur, pid });
    else writeRecipeInstance(workspacePath, { ...instance, pid });
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    writeRecipeInstance(workspacePath, {
      ...instance,
