...
/**
 * Gate: inbox write tools require a session ID (header or env).
 * This ensures only clawdevbox-spawned sessions can use inbox tools.
 */
function requireSessionId(extra: unknown): string {
  const sid = resolveAgentSessionId(extra as Parameters<typeof resolveAgentSessionId>[0]);
  if (!sid) {
    throw new Error(
      'inbox tools require a clawdevbox session context (X-Clawdevbox-Session-Id header or CLAWDEVBOX_SESSION_ID env). ' +
      'This tool is only available in sessions spawned by clawdevbox.',
    );
  }
  return sid;
}
...
const questionOptionSchema = z.object({
  ...
  isRecommended: z.boolean().optional().describe(
    'Mark this option as the agent\'s recommended choice. The UI highlights recommended options ' +
    'so the user can decide faster. Multiple options may be recommended when mode is "multi". ' +
    'STRONGLY ENCOURAGED — always indicate your recommendation when you have one.',
  ),
  rationale: z.string().max(500).optional().describe(
    'Brief explanation of tradeoffs for this option (e.g. "Fast but no ACID guarantees"). ' +
    'Shown as a secondary line below the label. STRONGLY ENCOURAGED — helps the user make ' +
    'informed decisions without needing to ask follow-ups.',
  ),
});
...
export function registerInboxEntries(ws: Workspace): void {
  ...
  defineTool({
    ...
    handler: async (args, extra) => {
      // Require a valid session context — only clawdevbox-spawned agents can use inbox.
      const headerSessionId = requireSessionId(extra);
        ...
        if (!notifications.enabled || !notifications.vapid) {
          ...
          push = await sendNotification(
            ...
            notifications.vapid,
            {
              ...
              body: pushBody,
              tag: `inbox:${item.id}`,
              url: `/inbox/${encodeURIComponent(item.id)}`,
            },
          );
        }
      }
