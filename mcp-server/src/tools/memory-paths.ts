 ...
 * and on-disk path layout (type/file + .events sidecar).
 */

import { join, dirname, basename, extname } from 'node:path';
import type { VaultInfo } from '../vault-chain.ts';

export type MemoryType = 'fact' | 'lesson' | 'session' | 'wiki';
...
export function vaultPathFor(
  ...
  return join(vaultMemoryRoot(vault), typeFolder(type), filename);
}
...
export function eventsPathFor(
  ...
  const eventsDir = dir === '.'
    ? join(vaultMemoryRoot(vault), typeFolder(type), '.events')
    : join(vaultMemoryRoot(vault), typeFolder(type), '.events', dir);
