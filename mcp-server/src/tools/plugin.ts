...
import { ensureClawdevboxSelfLink as ensureClawdevboxSelfLinkStub } from '../builtin-marketplace.ts';
 ...
 *  Exported so cli/start.ts can fire-and-forget the dev-self-link at boot. */
export function locateHostNodeModules(): string | null {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    if (existsSync(candidate)) {
      ensureClawdevboxSelfLink(candidate);
      return candidate;
    }
...
/**
 * Boot-time self-heal: delegate to the stub-package writer in
 * `builtin-marketplace.ts` so the boot path matches what `clawdevbox init`
 * writes. The previous local implementation here created a JUNCTION from
 * `<host_node_modules>/clawdevbox` back to the package root, which (a)
 * caused infinite directory recursion when copilot/agency CLI tree-copied
 * plugins through the junction, and (b) silently desynchronized with the
 * modern stub-package layout written by init — leaving the long-running
 * clawdevbox process holding a stale Node ESM `package.json` cache after
 * any init / plugin.install rewrote the stub (manifesting as
 * `Cannot find module '.../node_modules/clawdevbox/dist/agent-clis.mjs'`
 * during agency-provider resume).
 *
 * The stub-package version is idempotent: it leaves an existing valid
 * stub untouched and replaces any stale junction with a fresh stub.
 */
function ensureClawdevboxSelfLink(hostNodeModules: string): void {
  try {
    ensureClawdevboxSelfLinkStub(hostNodeModules);
  } catch {
    // best-effort; if symlink creation is denied, the user can vendor or
    // npm-install clawdevbox themselves
  }
}
