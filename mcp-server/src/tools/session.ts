...
export function registerSessionEntries(ws: Workspace): void {
  ...
  defineTool({
    ...
    parameters: z.object({
      ...
      context_tier: z.enum(['default', 'long_context']).optional().describe(
        "Context-window tier passed as --context (copilot/agency only; ignored by other providers). 'default' | 'long_context'.",
      ),
      reasoning_effort: z.enum(['none', 'minimal', 'low', 'medium', 'high', 'xhigh', 'max']).optional().describe(
        'Reasoning-effort tier passed as --effort (copilot/agency only; ignored by other providers).',
      ),
    ...
    handler: async (args) => {
      ...
      const result = await spawnDispatchOrResume(ctx, {
        ...
        context_tier: args.context_tier ?? null,
