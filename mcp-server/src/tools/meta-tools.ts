...
export function registerMetaTools(server: McpServer, toolsDir?: string): void {
  ...
  // --- Promoted tools --------------------------------------------------------
  // These tools are also accessible via run_tool, but are exposed as first-class
  // MCP tools so agents always see them without needing list_tools + learn_tool.
  const PROMOTED_TOOLS = ['skill.list', 'skill.read', 'search_memory', 'update_status'];

  const registry = getRegistry();
  for (const toolName of PROMOTED_TOOLS) {
    const entry = registry.get(toolName);
    if (!entry) continue;
    // Extract the zod shape from z.object(...) for registerTool's inputSchema format
    // The MCP SDK accepts either a ZodSchema or a record { paramName: ZodType }
    const shape = (entry.parameters as unknown as { shape: Record<string, z.ZodType> }).shape;
    server.registerTool(
      toolName,
      {
        description: entry.description,
      ...
      async (
        ...
        const parseResult = entry.parameters.safeParse(args ?? {});
        if (!parseResult.success) {
          const issues = (parseResult.error.issues as Array<{ path: (string | number | symbol)[]; message: string }>)
            .map((i) => `${i.path.join('.')}: ${i.message}`)
            .join('; ');
          return { isError: true, content: [{ type: 'text', text: `Validation error: ${issues}` }] };
        }
        try {
          const result = await entry.handler(parseResult.data, extra);
          if (result && typeof result === 'object' && 'content' in (result as object)) {
