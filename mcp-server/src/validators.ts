  ...
  if (r.agent !== undefined) {
    ...
    if (
      ...
      errors.push({
        path: 'agent',
        code: 'INVALID_VALUE',
        message: `agent must be a non-empty agent name (e.g. 'dev-buddy', 'incidents-investigator').`,
      });
    }
  }
  ...
  if (r.steps !== undefined) {
    if (!Array.isArray(r.steps)) {
      ...
      r.steps.forEach((step, i) => {
        ...
        // required — optional boolean. When true, the step cannot be skipped
        // (any `→ skipped` transition is rejected at runtime by the step
        // machine). Defaults to false (skippable).
        if (step.required !== undefined && typeof step.required !== 'boolean') {
          errors.push({ path: `${pathPrefix}.required`, code: 'TYPE', message: 'step.required must be a boolean.' });
        }

        // validation — optional block enabling the completion gate. When
        // present, the step reaches `done` only via a server-applied verifier
        // verdict (never a direct running→done). Accepts EITHER a single gate
        // object OR an array of gate objects (all must pass — AND).
        if (step.validation !== undefined) {
          if (Array.isArray(step.validation)) {
            if (step.validation.length === 0) {
              errors.push({ path: `${pathPrefix}.validation`, code: 'TYPE', message: 'step.validation array must contain at least one gate object.' });
            }
            step.validation.forEach((gate, j) => {
              const gatePath = `${pathPrefix}.validation[${j}]`;
              if (!isPlainObject(gate)) {
                errors.push({ path: gatePath, code: 'TYPE', message: 'validation gate must be an object.' });
                return;
              }
              const gm = gate.mode;
              if (typeof gm !== 'string' || !['evidence', 'artifacts', 'judge'].includes(gm)) {
                errors.push({ path: `${gatePath}.mode`, code: 'ENUM', message: 'validation.mode must be one of: evidence, artifacts, judge.' });
              }
              for (const vk of ['verifier_provider', 'verifier_agent', 'verifier_model']) {
                if (gate[vk] !== undefined && typeof gate[vk] !== 'string') {
                  errors.push({ path: `${gatePath}.${vk}`, code: 'TYPE', message: `${vk} must be a string.` });
                }
              }
            });
          } else if (!isPlainObject(step.validation)) {
            errors.push({ path: `${pathPrefix}.validation`, code: 'TYPE', message: 'step.validation must be an object or an array of gate objects.' });
          } else {
            ...
            for (const vk of ['verifier_provider', 'verifier_agent', 'verifier_model']) {
              if (step.validation[vk] !== undefined && typeof step.validation[vk] !== 'string') {
                errors.push({ path: `${pathPrefix}.validation.${vk}`, code: 'TYPE', message: `${vk} must be a string.` });
              }
            }
            if (step.validation.max_rework !== undefined && (typeof step.validation.max_rework !== 'number' || !Number.isInteger(step.validation.max_rework) || step.validation.max_rework < 1)) {
              errors.push({ path: `${pathPrefix}.validation.max_rework`, code: 'TYPE', message: 'validation.max_rework must be an integer >= 1.' });
            }
          }
        }

        // execution — optional block selecting how the step's own work runs.
        // Absent = inline (today's behavior).
        if (step.execution !== undefined) {
          if (!isPlainObject(step.execution)) {
            ...
            if (em !== undefined && (typeof em !== 'string' || !['inline', 'fresh-session'].includes(em))) {
