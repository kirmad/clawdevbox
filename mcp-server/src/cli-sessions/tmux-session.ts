...
export async function createTmuxSession(
  ...
  const spawnSession = async (cols: number, rows: number): Promise<'created' | 'adopted'> => {
    const baseArgs = [
      'new-session', '-d',
      '-s', sessionName,
      '-x', String(cols),
    ...
    if (r.exitCode !== 0) {
      ...
      // If the session already exists (e.g. from a prior clawdevbox that
      // crashed without cleanup), adopt it rather than failing outright.
      if (msg.includes('duplicate session')) {
        return 'adopted';
      }
      throw new Error(`tmux new-session failed: ${msg}`);
    }
    await waitForPane(client, sessionName, usingPsmux ? 100 : 0);
    return 'created';
  };

  const outcome = await spawnSession(opts.cols, opts.rows);

  // Tag the session with the creator clawdevbox PID via tmux's session
  // environment. Only for newly created sessions — adopted sessions keep
  // their original tag (the sweep will update it on next restart anyway).
  if (outcome === 'created') {
    try {
