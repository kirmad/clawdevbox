...
export interface SendArgs {
  ...
  agent?: string | null;
  model?: string | null;
  /** External field name: `context_tier`. Maps to SpawnSessionOpts.contextTier. */
  context_tier?: ContextTier | null;
  /** External field name: `reasoning_effort`. Maps to SpawnSessionOpts.reasoningEffort. */
...
export async function spawnDispatchOrResume(
  ...
  return withKeyedLock(`session.send:${sessionGuid}`, async () => {
    // 1. LIVE? → dispatch
    //
    // NOTE: model / context_tier / reasoning_effort are NOT applied on the
    // dispatch path. A live session is an already-running CLI process whose
    // --model/--context/--effort argv was fixed at spawn time; sending a
    // follow-up prompt cannot change it. These overrides only take effect on
    // the spawn and resume paths (both of which re-invoke provider.spawnSession
    // via runRecipe). Callers who need a different model must spawn/resume a
    // fresh session.
      ...
      if (provider?.supportsResume) {
        try {
          const result = await runResume(ctx, resumeRow, args.prompt, sessionGuid, {
            ...
            contextTier: args.context_tier ?? undefined,
