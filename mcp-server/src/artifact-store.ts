 ...
 *
 * Resolution order:
 *   1. `CLAWDEVBOX_PROJECT_DIR` (if set) — artifacts written under
 *      `<projectDir>/artifacts/<id>/` are discoverable without explicit
 *      workspace registration. Returned with workspaceId='project'.
 *   2. Each entry from `listWorkspaces(resolveWorkspacesRoot())`, in
 *      registry order.
 *
 * Returns null when the artifact id isn't present in any of those roots.
 */
export function findArtifact(id: string): FoundArtifact | null {
  ...
  try {
    ...
    // Registry unreadable — nothing more to try.
  }
  return null;
}
