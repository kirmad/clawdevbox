...
export interface RegisteredTrigger {
  ...
  type: string;
  /**
   * Optional human-readable label provided by the caller of
   * `trigger.instance.register` (e.g. "Teams chat listener",
   * "Hourly QoE sanity-check"). When present, the SPA shows this
   * instead of the opaque registered-id. Falls back to `id` for legacy
  ...
  const tx = conn.transaction((items: RegisteredTrigger[]) => {
    ...
    const upsert = conn.prepare(
      `INSERT INTO triggers (
         id, workspace_id, type, name, params_json,
       ...
       ON CONFLICT(id) DO UPDATE SET
         type = excluded.type,
         name = COALESCE(excluded.name, triggers.name),
