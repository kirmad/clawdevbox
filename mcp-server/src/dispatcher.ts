...
import type { ContextTier, ReasoningEffort } from './agent-clis/types.ts';
...
export class Dispatcher {
  ...
  async dispatchToInstance(
    ...
    | { status: 'ok'; state: 'dispatched'; dispatchId: string }
  > {
    // Resolve the live target. tmux sessions (copilot/claude/echo) live in
    // `tmuxSessionRegistry`. Legacy IPty providers (agency-provider, etc.)
    // live in `pty-registry`. Both paths support raw-byte send with the
    // same Escape / text / Enter sequence — we route based on which
    // registry has the live handle.
    const tmuxSession = tmuxSessionRegistry.get(instanceId);
    let target:
      | { kind: 'tmux'; session: typeof tmuxSession }
      | { kind: 'pty' }
      | null = null;
    if (tmuxSession) {
      target = { kind: 'tmux', session: tmuxSession };
    } else {
      const { isSessionLive } = await import('./pty-registry.ts');
      if (isSessionLive(instanceId)) target = { kind: 'pty' };
    }
    ...
    try {
      await withKeyedLock(`dispatch:${instanceId}`, async () => {
        // Native follow-up queue vs submit. Copilot/Agency expose a Ctrl+Q
        // FIFO that commits the input to the CLI's queue whether the agent is
        // idle OR mid-turn; the CLI drains it after the current turn.
        // Empirically verified: typing a prompt + Ctrl+Q into a BUSY copilot
        // shows "Queued (1)" and the agent answers it once free. So for
        // queue-capable agents we do NOT wait for idle (which can hang, or
        // time out and then drop a submit into a mid-turn TUI) — we queue.
        const useQueue = target.kind === 'tmux' && !!target.session
          && (await this.resolveQueueMode(instanceId)) === 'ctrl-q';

        if (!useQueue) {
          // Submit path only: wait for idle (provider-opt-in via copilot
          // events.jsonl) so we don't type into a mid-turn TUI that drops it.
        ...
        if (useQueue && target.kind === 'tmux' && target.session) {
          // QUEUE via Ctrl+Q. Ctrl+Q commits the INPUT BOX content to the
          // CLI's native FIFO — reliable whether the agent is idle OR busy,
          // with no idle-wait. Empirically: a MULTI-LINE prompt sent via
          // bracketed paste is NOT committed by Ctrl+Q (paste lands but Ctrl+Q
          // queues nothing), whereas single-line LITERAL input is. So we
          // flatten newlines to a separator — keeping it a single literal line
          // that sendText types directly — then Ctrl+Q queues it.
          const flat = prompt.replace(/\r?\n[\s]*/g, '  \u00b7  ').trim();
          try {
            await target.session.sendText(flat);
            await sleepP(250);
            await target.session.sendKey('C-q');
            dispatched = true;
            logger.info({ instanceId }, 'dispatcher: queued prompt via native Ctrl+Q');
          } catch (err) {
            logger.warn(
              { instanceId, err: err instanceof Error ? err.message : String(err) },
              'dispatcher: Ctrl+Q queue send failed — target likely gone',
            );
          }
        } else {
          // SUBMIT path (agents without a native queue, or legacy pty).
          // Bytes-on-the-wire ordering. Each gap is empirically required:
          // Copilot's TUI absorbs Enter that arrives too close to a preceding
          // ESC or the input bytes. Retry up to 3× with verified receipt.
          const MAX_ATTEMPTS = 3;
          const WAKE_DELAYS = [300, 500, 1000]; // increasing delays between retries
          // Snapshot the tmux pane BEFORE sending so confirmDispatchReceipt can
          // prove the keystrokes landed (the pane changes) vs silently no-op'd
          // against a stale/unregistered instance.
          const beforeSnap = target.kind === 'tmux'
            ? await target.session!.snapshot().catch(() => null)
            : null;

          for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            const wakeDelay = WAKE_DELAYS[Math.min(attempt, WAKE_DELAYS.length - 1)];
            if (target.kind === 'tmux') {
              await target.session!.sendKey('Escape');
              await sleepP(wakeDelay);
              await target.session!.sendText(prompt);
              await sleepP(250);
              await target.session!.sendKey('Enter');
            } else {
              const { writeToPty } = await import('./pty-registry.ts');
            ...
            // Verify receipt
            const confirmed = await this.confirmDispatchReceipt(instanceId, target, beforeSnap);
            if (confirmed) {
              dispatched = true;
              if (attempt > 0) {
                logger.info({ instanceId, attempt }, 'dispatcher: dispatch confirmed on retry');
              }
              break;
            }
            logger.warn(
              { instanceId, attempt, maxAttempts: MAX_ATTEMPTS },
              'dispatcher: dispatch not confirmed, retrying',
            );
          }
        }
        ...
        if (!dispatched) {
          logger.error(
            { instanceId },
            'dispatcher: dispatch failed — agent did not receive input',
          );
    ...
    // Mark the session as 'working' so the idle-reaper doesn't kill it
    // while copilot is processing the dispatched prompt. Without this,
    // a stale derived_state_at from before the dispatch would make the
    // session look idle for > 15 min and trigger a reap.
    try {
      this.db.prepare(
        `UPDATE agent_sessions SET derived_state = 'working', derived_state_at = ?
  ...
  /**
   * The provider's follow-up queue mode ('ctrl-q' | 'none') for a live
   * instance, resolved from its agent_cli. Deliberately NOT filtered by
   * status — the CLI's queue capability is stable across resumes even when
   * the DB row's status has drifted. Returns undefined when the provider is
   * unknown.
   */
  private async resolveQueueMode(instanceId: string): Promise<'ctrl-q' | 'none' | undefined> {
    const row = this.db
      .prepare(
        `SELECT agent_cli FROM agent_sessions
         WHERE recipe_instance_id = ?
         ORDER BY started_at DESC LIMIT 1`,
      )
      .get(instanceId) as { agent_cli: string } | undefined;
    if (!row?.agent_cli) return undefined;
    return this.ws.agentCliProviders?.get(row.agent_cli)?.capabilities?.queueMode;
  }
  ...
  /**
   * Confirm that a dispatch actually landed in the agent.
   *
   * Two independent signals, because different agent CLIs expose different
   * evidence:
   *   1. Copilot writes `~/.copilot/session-state/<id>/events.jsonl`; a new
   *      event after the send proves receipt.
   *   2. Every other tmux agent (claude, echo, …) has NO such file, so we
   *      compare the tmux pane BEFORE vs AFTER the send — if the keystrokes
   *      landed the pane changes (the prompt echoes / the agent starts
   *      working); if the write silently no-op'd (e.g. against a stale,
   *      unregistered instance after a restart) the pane is byte-identical.
   *
   * Returning false makes `dispatchToInstance` report `target_unavailable`,
   * which makes the caller fall through to RESUME a fresh session — the
   * behaviour that recovers a conversation whose live terminal went stale.
   * Previously this method returned `true` whenever the copilot file was
   * absent, so a dropped dispatch to a claude agent looked successful and the
   * reviewer's question/comment never reached the agent.
   */
  private async confirmDispatchReceipt(
    instanceId: string,
    target?: { kind: 'tmux'; session: { snapshot(opts?: { ansi?: boolean }): Promise<string> } | null | undefined } | { kind: 'pty' },
    ...
    const row = this.db
      ...
      .get(instanceId) as { cli_session_id: string | null } | undefined;

    // 1. Copilot events.jsonl growth (when present).
    if (row?.cli_session_id) {
      const { homedir } = await import('node:os');
      const { join } = await import('node:path');
      const eventsPath = join(homedir(), '.copilot', 'session-state', row.cli_session_id, 'events.jsonl');
      ...
      if (existsSync(eventsPath)) {
        ...
        for (let i = 0; i < 10; i++) {
          await sleepP(500);
          try {
            if (statSync(eventsPath).size > sizeBefore) return true;
          } catch { break; }
        }
        return false; // copilot file exists but never grew — dispatch dropped
      }
    }

    // 2. Non-copilot tmux agent: verify the pane changed after the send.
    if (target?.kind === 'tmux' && target.session && beforeSnap != null) {
      for (let i = 0; i < 10; i++) {
        await sleepP(500);
        try {
          const now = await target.session.snapshot();
          if (now !== beforeSnap) return true; // keystrokes landed
        } catch { break; }
      }
      return false; // pane never changed — keystrokes did not land
    }

    // No probe available (legacy IPty path, or snapshot failed) — preserve the
    // old lenient behaviour rather than force a needless resume.
    return true;
  }

  /**
   *
   * **tmux is the source of truth.** A session is considered live iff a
   * `cdb_<recipe_instance_id>` tmux session is alive RIGHT NOW (via
   * `tmuxSessionRuntime.list()`), regardless of what the DB row's `status`
   * column says. The DB `status` can drift across crashes / restarts /
   * idle-reaper races; tmux cannot lie.
   *
   * Effects: when we discover a live tmux session that isn't yet in
   * `tmuxSessionRegistry` (e.g. boot adoption hadn't yet completed,
   * or a long-running process is querying after a registry reset),
   * we adopt it and register so subsequent calls hit the in-memory map.
   *
  ...
  async findLiveInstanceForSession(cliSessionId: string): Promise<string | null> {
    // Delegates to the shared resolver (see live-instance-resolver.ts).
    // Kept as a thin method so existing dispatcher callers/tests keep working.
    const { resolveLiveInstanceForSession } = await import('./live-instance-resolver.ts');
    return resolveLiveInstanceForSession(this.db, cliSessionId);
  }
  ...
  async spawnFromCallback(
    ...
    overrides: {
      ...
      model?: string;
      /** Optional --context override for copilot/agency (ignored elsewhere). */
      contextTier?: ContextTier;
      /** Optional --effort override for copilot/agency (ignored elsewhere). */
  ...
  > {
    ...
    try {
      const result = await runRecipe({
        ...
        contextTier: overrides.contextTier,
  ...
  private recordFailure(fire: FireRow, trigger: TriggerRow | undefined, errStr: string): void {
    ...
    if (current.attempt < current.max_attempts) {
      ...
      try {
        deadLetterFire(this.db, fire, {
          error: errStr,
          title: `Trigger fire failed permanently: ${trigger?.id ?? fire.trigger_id ?? '(unknown)'}`,
          preview: errStr.slice(0, 200),
        });
      } catch (err) {
        logger.warn(
          { err: err instanceof Error ? err.message : String(err) },
