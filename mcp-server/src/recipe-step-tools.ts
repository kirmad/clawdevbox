...
export function updateStatusImpl(
  ...
  // becomes the claimed evidence; the server validation worker-loop then
  // verifies it. This keeps gating transparent — recipe authors just add a
  // `validation:` block, and agents keep calling update_status(done) as usual.
  // Only invalid paths (e.g. validating→done without a verdict) still hit the
  // gate error. viaVerdict is never set by this tool, so the finalize path
  // (applyVerdict) is unaffected.
  const isGatedStep = current.validation_json != null;
  const isAutoClaim =
    isGatedStep &&
    opts.status === 'done' &&
    (current.status === 'running' || current.status === 'awaiting_user');
  const claimNote = isAutoClaim
    ? 'Claim recorded — this step is validation-gated. A verifier is now checking your work. '
      + 'You will be notified with the next step (on pass) or with required fixes (on fail). '
      + 'This step is NOT done yet; do not proceed.'
    : undefined;
  const effectiveStatus = isAutoClaim ? 'validating' : opts.status;
  ...
  const tx = db.transaction(() => {
    ...
    // Re-claim rotation: an auto-claim on a step that ALREADY carries a prior
    // verdict is a RE-claim (the agent reworked a FAIL and is re-submitting).
    // Rotate to a fresh validation attempt — bump validation_attempt (⇒ a fresh
    // verdict-file path), clear the stale verifier session, and clear the prior
    // verdict — so the worker spawns a FRESH verifier instead of re-reading the
    // stale FAIL verdict file. A FIRST claim has verdict_json == null and does
    // NOT rotate (attempt stays 0). step_events retains the prior verdict audit.
    if (isAutoClaim && current.verdict_json != null) {
      db.prepare(
        `UPDATE recipe_steps
           SET validation_attempt = validation_attempt + 1,
    ...
    if (
      ...
      try {
        workingRow = transitionStatus(db, current.id, {
          ...
          message: claimNote ?? opts.message,
          state: opts.state,
        ...
        if (e instanceof StepRequiredError) {
          throw new ToolErrorBox({
            code: 'STEP_REQUIRED',
            message: e.message,
    ...
    if (TERMINAL.has(workingRow.status)) {
      ...
      if (allTerminal && siblings.length > 0) {
        out.recipe_instance_status = cascadeInstanceIfAllTerminal(db, opts.recipe_instance_id);
      }
    }
