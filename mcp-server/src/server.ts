...
import { registerPrWalkthroughEntries } from './tools/pr-walkthrough.ts';
...
// NOTE: clawdevbox does not ship a built-in MCP `instructions` field.
// Agent personas + tool-access protocol live in plugin agent definitions
// (e.g. plugins/dev-buddy/agents/dev-agent.agent.md), which the CLI client
// loads directly. The MCP server is intentionally persona-agnostic so any
// client — with or without the dev-buddy plugin — can connect cleanly.
...
function registerAllBuiltinEntries(ws: Workspace): void {
  ...
  registerPrWalkthroughEntries(ws);
}
