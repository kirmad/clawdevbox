  ...
  proc.onData((chunk) => {
    ...
    for (const m of chunk.matchAll(URL_RE)) {
      ...
      if (u.includes('-inspect.')) {
        ...
        // Only accept the URL for OUR port (opts.port, typically 5201).
        // The tunnel may expose multiple ports (e.g. 5201 + 5299) and
        // we don't want the share-server port URL leaking into the
        // primary tunnel status.
        const portMatch = u.match(/-(\d+)\./);
        if (portMatch && Number(portMatch[1]) !== opts.port) continue;
        if (!current.url || current.url !== u) {
          current.url = u;
...
export function deriveTunnelName(projectDir: string): string {
  // Include hostname so multiple machines under the same devtunnel account
  // get unique tunnel names (e.g. "clawdevbox-desktop1", "clawdevbox-laptop2").
  const host = hostname().toLowerCase().replace(/[^a-z0-9-]/g, '').slice(0, 15);
  ...
  // Append hostname for uniqueness across machines
  const name = host ? `${head}-${host}` : head;
  // devtunnel names are typically <=60 chars; trim hard.
  return name.slice(0, 50) || 'clawdevbox';
}
