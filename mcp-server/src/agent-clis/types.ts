...
/**
 * Context-window tier requested for a spawned session. Consumed ONLY by
 * providers that expose a `--context` flag (Copilot and Agency, which wraps
 * Copilot). Claude / echo-stub / e2e-test-runner ignore it.
 *
 * - `default`: the provider's normal context window.
 * - `long_context`: opt into the provider's extended context window.
 */
export type ContextTier = 'default' | 'long_context';

/**
 * Reasoning-effort tier requested for a spawned session. Consumed ONLY by
 * providers that expose an `--effort` flag (Copilot and Agency). Other
 * providers ignore it.
 */
...
/** Allowed `contextTier` values (for runtime validation at API boundaries). */
export const CONTEXT_TIERS: readonly ContextTier[] = ['default', 'long_context'] as const;

/** Allowed `reasoningEffort` values (for runtime validation at API boundaries). */
...
export interface SpawnSessionOpts {
  ...
  model?: string;
  /**
   * Optional context-window tier. Maps to the CLI's `--context <tier>` flag.
   * Consumed ONLY by copilot and agency (which forwards to copilot). All
   * other providers ignore this field. When unset, copilot defaults to
   * `long_context` (see copilot.ts). Values: `default` | `long_context`.
   */
  contextTier?: ContextTier;
  /**
   * Optional reasoning-effort tier. Maps to the CLI's `--effort <level>`
   * flag. Consumed ONLY by copilot and agency. All other providers ignore
   * this field. When unset, copilot defaults to `max` (see copilot.ts).
   */
