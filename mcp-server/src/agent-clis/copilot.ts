...
export const copilotProvider: AgentCliProvider = {
  ...
  async spawnSession(ctx: ProviderCtx, opts: SpawnSessionOpts): Promise<AgentHandle> {
    ...
    //
    // `--no-ask-user` suppresses Copilot's tool-confirmation prompts entirely.
    // It must stay immediately adjacent to (and before) `--yolo`: together they
    // form the paired autonomy contract — `--no-ask-user` stops the prompt from
    // ever appearing, `--yolo` grants the permissions that would otherwise be
    // requested. Keep them adjacent so the pairing is obvious and unbroken.
    //
    ...
    // a high-quality model + extended context + max reasoning effort UNLESS
    // the caller explicitly overrides. Each flag is emitted EXACTLY once —
    // the `??` fallback means an explicit override always wins over the
    // default and never produces a duplicate flag. Agency (which wraps
    // copilot) applies the same contract in its own provider.
    const model = opts.model ?? 'claude-opus-4.8';
    const contextTier = opts.contextTier ?? 'long_context';
    const reasoningEffort = opts.reasoningEffort ?? 'max';
    argv.push('--model', model);
    argv.push('--context', contextTier);
    argv.push('--effort', reasoningEffort);
