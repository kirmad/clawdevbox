...
import TerminalsTiledView from './TerminalsTiledView.vue';
...
// The Main Agent has its own dedicated top-level tab (MainAgentPanel),
// so we hide it from this list — one source of truth per session.
const activeSessions = computed(() =>
  store.terminals.items.filter((s) => s.live && s.instance_id !== 'main'),
);
...
const recentArchived = computed(() => {
  ...
  return store.terminals.items.filter(
    (s) => !s.live && s.instance_id !== 'main' && (s.ended_at ?? s.started_at) >= cutoff,
  );
});
...
// ---------------------------------------------------------------------------
// View mode: 'list' = legacy (tab list + one large xterm + side panel).
// 'tiles' = grouped-by-workspace dynamic tile grid (TerminalsTiledView).
// Persisted across reloads so the user's preferred view sticks.
// ---------------------------------------------------------------------------
const VIEW_MODE_KEY = 'clawdevbox.terminals.viewMode';
...
function setViewMode(mode: ViewMode): void {
  ...
  // The two views have very different geometries; nudge xterm refits.
  setTimeout(() => window.dispatchEvent(new Event('resize')), 50);
}
function onTileFocus(instanceId: string): void {
  // Switch back to the single-terminal interactive view, focused on
  // the clicked tile — mirrors tmux's "zoom in on this pane".
  store.selectTerminal(instanceId);
  setViewMode('list');
}
// Include 'main' in the tiled view so the Main Agent appears in the
// project workspace tile (the user wants "main window will be primary
// space, remaining will be secondary windows"). The List view still
// hides 'main' since MainAgentPanel owns its dedicated tab there.
const tiledSessions = computed(() =>
  store.terminals.items.filter((s) => s.live),
);
...
const LS_TAB_COLLAPSED = 'clawdevbox.terminals.tabListCollapsed';
...
const tabListCollapsed = ref<boolean>(loadBool(LS_TAB_COLLAPSED, false));
...
watch(tabListCollapsed, (v) => {
  try { localStorage.setItem(LS_TAB_COLLAPSED, v ? '1' : '0'); } catch { /* ignore */ }
  scheduleRefit();
});
...
async function copyAttach(instanceId: string): Promise<void> {
  const cmd = `tmux attach -t cdb_${instanceId}`;
  try {
...
<template>
  <div class="terminals-shell">
    <header class="terminals-toolbar">
      <div class="view-toggle" role="group" aria-label="Terminals view mode">
        <button
          type="button"
          class="view-toggle__btn"
          :class="{ 'is-active': viewMode === 'list' }"
          title="List view: tab list on the left, one large xterm on the right"
        ...
        <button
          ...
          title="Tiled view: live terminals grouped by workspace in a dynamic grid (main agent + spawned sub-agents visible at the same time)"
          @click="setViewMode('tiles')"
        >
          <i class="pi pi-th-large" /> Group by workspace
        </button>
      </div>
      <button
        v-if="viewMode === 'tiles'"
    ...
    <TerminalsTiledView
      ...
      @focus-session="onTileFocus"
    />

    <div v-else class="terminals-panel">
    <aside v-show="!tabListCollapsed" class="tab-list" :style="{ flexBasis: tabListWidth + 'px' }">
      ...
      >
        ...
        <button
          ...
          @click.stop="copyAttach(s.instance_id)"
        >⎘</button>
        <button
    ...
    />
    <div v-if="tabListCollapsed" class="tab-list-expand-strip">
      <button class="tab-list-expand-btn" title="Show session list" @click="tabListCollapsed = false">
...
.view-toggle__btn:hover { color: var(--p-text-color); }
.view-toggle__btn.is-active {
  background: #2a2e38;
  color: var(--p-text-color);
}
.view-toggle__btn i { font-size: 11px; }
.toolbar-spawn {
  margin-left: auto;
  background: transparent;
...
.toolbar-spawn:hover { background: #20232c; color: var(--p-text-color); }
.toolbar-spawn i { font-size: 11px; }

.terminals-panel { display: flex; flex: 1 1 auto; min-height: 0; min-width: 0; }
.tab-list {
...
.tab-list-expand-btn:hover { background: rgba(255,255,255,0.06); color: var(--p-text-color); }
.tab-list-expand-btn i { font-size: 13px; }
.side-pane-host {
  flex: 0 0 auto;       /* width controlled via inline flex-basis */
  display: flex;
  min-width: 0;
...
.attach-btn { position: absolute; right: 28px; top: 50%; transform: translateY(-50%); padding: 2px 7px; font-size: 11px; background: #1a2a2c; color: #6cb4f7; border: 1px solid #22445a; border-radius: 3px; cursor: pointer; display: none; line-height: 1.4; }
.attach-btn:hover { background: #224466; color: #fff; }
.tab-row:hover .attach-btn { display: inline-block; }
