...
const LS_RAIL_COLLAPSED = 'clawdevbox.triggers.railCollapsed';
...
function triggerLabel(t: RegisteredTrigger): string {
  const name = (t.name ?? '').trim();
  if (name.length > 0) return name;
  return t.id;
}

function showSecondaryId(t: RegisteredTrigger): boolean {
    ...
    <template v-if="isDesktop">
        ...
        <div class="list-rail">
          ...
          >
            <div class="card-row title-row">
              <span class="title-text">{{ triggerLabel(t) }}</span>
              <Tag v-if="!t.enabled" value="off" severity="secondary" />
            </div>
            <div v-if="showSecondaryId(t)" class="card-sub-id">{{ t.id }}</div>
            <div class="card-row">
      ...
      <div class="detail-col">
        <div v-if="railCollapsed" class="rail-expand-strip">
          <button class="rail-expand-btn" title="Show triggers list" @click="toggleRail">
            <i class="pi pi-angle-right" />
            <span class="rail-expand-label">Triggers</span>
...
/* Mono technical id as subtitle (shown only when a human name exists). */
.card-sub-id {
