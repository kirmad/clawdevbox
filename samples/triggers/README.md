...
The reference comment-watchers (`ado-comment-watcher.ts/.py`) use Mode B because a single cron tick can surface multiple new comments. The `mock-conductor` test harness in `test/` exercises both modes — Mode A's lone `callback` and Mode B's live POSTs both feed the same captured-callbacks list.
...
- **The final summary uses Mode A.** When the budget is reached (or the PR closes), the script sets ONE summary `callback` on stdout and exits. Conductor delivers it as the script's natural handoff — the agent reads "watch ended, here's why" and decides what to do next.

The on-receiving-end agent can't tell the difference: every callback is a `{ prompt, context }` body, regardless of which path delivered it. The two modes coexist on the same `callback_url` because Conductor's callback fan-out is path-keyed, not source-keyed.
...
t=8s   write { state, callback: <summary>, systemMessage } to stdout, exit 0.
       │
       └─[Mode A]─→ Conductor reads stdout, delivers the one `callback`
                    entry to env.callback_url internally.
```

End result on the agent's resume queue: **3 messages**. Comments delivered live during the watch (Mode B); summary delivered on exit (Mode A). The mock-conductor in `test/` validates both paths with Scenario F.
...
**Triggers are language-agnostic.** Both files implement the exact same trigger using the same stdin/stdout/env-var/HTTP protocol. Pick whichever language fits your operational preferences. Conductor doesn't care — it just spawns a subprocess and pipes JSON.

To switch from TS to Python, only one line in `triggers.json` changes:

```diff
- "command": "tsx $CONDUCTOR_PROJECT_DIR/.conductor/triggers/_runtime/ado-comments-thr_01HX.ts"
+ "command": "python3 $CONDUCTOR_PROJECT_DIR/.conductor/triggers/_runtime/ado-comments-thr_01HX.py"
```
...
When a trigger is registered, Conductor mints a structured callback URL based on what the trigger should do:
...
The trigger's registration tells Conductor which URL shape to mint. The script gets it ready-to-use as `env.callback_url` and POSTs (Mode B) or returns `callback.body` (Mode A) without thinking about routing.
...
`callback.body` is exactly the same shape a Mode-B script would POST to `env.callback_url`. Conductor delivers Mode A's lone entry on the script's behalf.
...
ado-comment-watcher.ts (Mode B):
  ...
  - writes JSON to stdout: { state, systemMessage }, exits 0
        │
        ▼
Conductor sidecar /callback/threads/thr_01HX/resume (per POST):
...
1. Drop the file in: `<workspace>/.conductor/triggers/_runtime/ado-comments-<thread_id>.ts`
2. The agent (or you) calls `trigger.upsert(...)` to register the entry. Conductor mints the right callback URL based on what the trigger declares it does (here: `subscriber_thread_id` set → resume URL).
3. Set ADO credentials in Conductor's keychain.
...
Because `ado-comment-watcher.ts` uses Mode B, it needs a callback endpoint to POST to during the run. The `test/capture.mjs` helper spins up a local `mock-conductor` first, then pipes a synthetic envelope through the script, then prints the resulting timeline:

```bash
cd test && node capture.mjs
# → STDIN envelope, STDOUT response, exit code, AND the live Mode B POSTs
#   captured by mock-conductor at /test/received-callbacks
```
...
The same one-URL pattern applies elsewhere — only the registration shape (which URL Conductor mints) varies:
...
|---|---|---|
| `state` (in stdin envelope, returned via stdout) | Small structured data — counters, ids, timestamps, settings. The example here uses it for `lastCommentId`, `selfUser`, etc. | Conductor reads the previous value from `triggers.state_json` and gives it to the script; the script returns the new value via stdout JSON; Conductor persists. |
| `trigger_data_dir` | Files, caches, large blobs, anything you'd `fs.writeFile`. Path is `<project_dir>/.conductor/triggers/<trigger_id>/data/`. | The script writes/reads with normal `fs` calls. Conductor creates the directory on first run and (for hot triggers) deletes it when the trigger goes away. Per-trigger — no collisions with other triggers. |
...
- In Mode A, callback delivery happens *after* the script exits — Conductor handles delivery failures and retries; the script doesn't need to.
- In Mode B, a failed POST means the script exits non-zero; Conductor logs `last_run_error` and does NOT update state — next tick retries.

## Why this design

The earlier drafts of this script:
1. Made 3-5 MCP calls per comment to look up threads, append messages, wake CLIs.
2. Hardcoded routing fields (`thread_id`, `recipe`, `parent_thread_id`) into the POST body, forcing the script to know Conductor's data model.

Now:
1. Routing is in the URL (minted by Conductor at registration time, not script-time).
